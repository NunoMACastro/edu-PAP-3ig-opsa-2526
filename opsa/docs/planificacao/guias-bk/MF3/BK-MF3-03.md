...
- `guia_path`: `docs/planificacao/guias-bk/MF3/BK-MF3-03.md`
- `last_updated`: `2026-04-13`

## O que vamos fazer neste BK
Implementar e documentar o requisito `RF39` associado a `Importar extratos bancarios (CSV/OFX) e fazer reconciliacao automatica.`, com entrega verificavel e rastreabilidade 1:1 entre backlog, matriz e guia.
...
- Impacto funcional: `Importar extratos bancarios (CSV/OFX) e fazer reconciliacao automatica.` desbloqueia valor direto no produto OPSA.
- Impacto tecnico: reforca a dimensao `Integracoes & Interoperabilidade` sob o criterio `Contratos e interoperabilidade com servicos externos, formatos e APIs definidos.`.
...
- Onde tocar no projeto: Contratos de integracao (API/eventos/ficheiros), retries, idempotencia e tratamento de falhas.
...
- O requisito `RF39` e demonstrado em cenario real de utilizacao.
...
- Dependencias tecnicas declaradas: `BK-MF3-02, BK-MF1-02, BK-MF1-09`.
...
1. Ler `RF/RNF` do BK (RF39) e confirmar contexto tecnico antes de tocar no codigo.
2. Validar dependencias declaradas (BK-MF3-02, BK-MF1-02, BK-MF1-09) e confirmar pre-condicoes em ambiente local.
3. Escrever mini-plano de implementacao (entrada, processamento, saida, erros) em 5-8 linhas.
4. Mapear onde alterar no projeto (frontend/backend/dados/integracao) para o BK `BK-MF3-03`.
5. Formalizar contrato de integracao e validar retries, idempotencia e mapeamento de erros.
...
### Negativos
- [ ] Resposta externa fora de schema e tratada sem quebrar fluxo interno.
- [ ] Evento duplicado nao gera processamento duplicado (idempotencia).
- [ ] Timeout de integracao usa retry/backoff e devolve estado previsivel.
...
## Criterios de aceite
- `Contrato de integracao documenta entradas, saidas e erros esperados.`
- `Retries/idempotencia estao definidos para falhas transitorias.`
- `Interoperabilidade validada com evidencia de chamada real/simulada.`
