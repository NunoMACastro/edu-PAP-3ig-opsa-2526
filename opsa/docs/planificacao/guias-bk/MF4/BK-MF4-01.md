# BK-MF4-01 - Gerar insights automaticos (tendencias, riscos, clientes, artigos parados).
...
- `guia_path`: `docs/planificacao/guias-bk/MF4/BK-MF4-01.md`
- `last_updated`: `2026-04-13`

## O que vamos fazer neste BK
Implementar e documentar o requisito `RF49` associado a `Gerar insights automaticos (tendencias, riscos, clientes, artigos parados).`, com entrega verificavel e rastreabilidade 1:1 entre backlog, matriz e guia.
...
- Impacto funcional: `Gerar insights automaticos (tendencias, riscos, clientes, artigos parados).` desbloqueia valor direto no produto OPSA.
- Impacto tecnico: reforca a dimensao `IA/Explicabilidade/Guardrails` sob o criterio `Regras de IA, explicabilidade, guardrails e limites de uso definidos por contexto.`.
...
- Onde tocar no projeto: Regras de IA, explicabilidade, limites de uso e guardrails para respostas confiaveis.
...
- O requisito `RF49` e demonstrado em cenario real de utilizacao.
...
- Dependencias tecnicas declaradas: `BK-MF3-10`.
...
1. Ler `RF/RNF` do BK (RF49) e confirmar contexto tecnico antes de tocar no codigo.
2. Validar dependencias declaradas (BK-MF3-10) e confirmar pre-condicoes em ambiente local.
3. Escrever mini-plano de implementacao (entrada, processamento, saida, erros) em 5-8 linhas.
4. Mapear onde alterar no projeto (frontend/backend/dados/integracao) para o BK `BK-MF4-01`.
5. Implementar guardrails, justificacao de resposta e limites de atuacao da IA por contexto.
...
### Negativos
- [ ] Prompt inseguro e bloqueado por guardrail explicito.
- [ ] Resposta sem fonte/justificacao minima e rejeitada para revisao.
- [ ] Pedido fora de politica retorna resposta limitada e auditavel.
...
## Criterios de aceite
- `Resposta de IA inclui explicabilidade e origem quando aplicavel.`
- `Guardrails bloqueiam pedidos fora de politica com resposta controlada.`
- `Limites de uso e intervencao humana estao claros e verificaveis.`
