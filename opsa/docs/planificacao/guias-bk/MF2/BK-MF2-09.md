# BK-MF2-09 - Relatorios e filtros por centro de custo/segmento.
...
- `estado`: `TODO`
- `esforco`: `S`
- `dependencias`: `BK-MF2-08`
- `rf_rnf`: `RF33`
- `fase_documental`: `Fase 1`
- `sprint`: `S04-S06`
- `core_or_reforco`: `Core`
- `proximo_bk`: `BK-MF2-10`
- `guia_path`: `docs/planificacao/guias-bk/MF2/BK-MF2-09.md`
- `last_updated`: `2026-04-13`

## O que vamos fazer neste BK
Implementar e documentar o requisito `RF33` associado a `Relatorios e filtros por centro de custo/segmento.`, com entrega verificavel e rastreabilidade 1:1 entre backlog, matriz e guia.
...
- Impacto funcional: `Relatorios e filtros por centro de custo/segmento.` desbloqueia valor direto no produto OPSA.
- Impacto tecnico: reforca a dimensao `Frontend (Nao Visual)` sob o criterio `Fluxos de UI-API, estado, validacoes e navegacao funcional claramente definidos.`.
...
- Onde tocar no projeto: Fluxo de estado, validacoes, navegacao e integracao UI-API no frontend React/Next.
...
- O requisito `RF33` e demonstrado em cenario real de utilizacao.
...
- Dependencias tecnicas declaradas: `BK-MF2-08`.
...
1. Ler `RF/RNF` do BK (RF33) e confirmar contexto tecnico antes de tocar no codigo.
2. Validar dependencias declaradas (BK-MF2-08) e confirmar pre-condicoes em ambiente local.
3. Escrever mini-plano de implementacao (entrada, processamento, saida, erros) em 5-8 linhas.
4. Mapear onde alterar no projeto (frontend/backend/dados/integracao) para o BK `BK-MF2-09`.
5. Implementar estado (loading/sucesso/erro), validacoes de formulario e integracao robusta com API.
...
### Negativos
- [ ] Formulario invalido bloqueia submissao e destaca campos com erro.
- [ ] Falha/timeout de API apresenta estado de erro recuperavel sem crash.
- [ ] Refresh de pagina preserva estado essencial sem inconsistencias.
...
## Criterios de aceite
- `Fluxo UI-API cobre loading, sucesso e erro com comportamento previsivel.`
- `Validacoes de entrada evitam submissao invalida sem bloquear fluxo normal.`
- `Navegacao e estado da pagina permanecem consistentes apos acoes principais.`
