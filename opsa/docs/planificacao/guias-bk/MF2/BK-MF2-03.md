# BK-MF2-03 - Movimentos de stock: entradas, saidas, transferencias, devolucoes.
...
- `guia_path`: `docs/planificacao/guias-bk/MF2/BK-MF2-03.md`
- `last_updated`: `2026-04-13`

## O que vamos fazer neste BK
Implementar e documentar o requisito `RF27` associado a `Movimentos de stock: entradas, saidas, transferencias, devolucoes.`, com entrega verificavel e rastreabilidade 1:1 entre backlog, matriz e guia.
...
- Impacto funcional: `Movimentos de stock: entradas, saidas, transferencias, devolucoes.` desbloqueia valor direto no produto OPSA.
- Impacto tecnico: reforca a dimensao `Base de Dados` sob o criterio `Persistencia, historico, consistencia e recuperacao (backup/restore) documentadas.`.
...
- Onde tocar no projeto: Modelacao de entidades, migracoes, consistencia transacional e persistencia em PostgreSQL/Redis.
...
- O requisito `RF27` e demonstrado em cenario real de utilizacao.
...
- Dependencias tecnicas declaradas: `BK-MF0-11, BK-MF0-12`.
...
1. Ler `RF/RNF` do BK (RF27) e confirmar contexto tecnico antes de tocar no codigo.
2. Validar dependencias declaradas (BK-MF0-11, BK-MF0-12) e confirmar pre-condicoes em ambiente local.
3. Escrever mini-plano de implementacao (entrada, processamento, saida, erros) em 5-8 linhas.
4. Mapear onde alterar no projeto (frontend/backend/dados/integracao) para o BK `BK-MF2-03`.
5. Aplicar schema/migracoes e validar consistencia de dados, indexes criticos e estrategia de rollback.
...
### Negativos
- [ ] Migracao falhada executa rollback sem perda de consistencia.
- [ ] Restauro de backup em ambiente de teste recupera registos esperados.
- [ ] Violacao de unicidade gera erro claro e nao cria estado parcial.
...
## Criterios de aceite
- `Schema/migracoes aplicados com sucesso e com plano de rollback testado.`
- `Consistencia de dados validada com queries objetivas e sem orfaos.`
- `Backup/restauro testado com evidencias de recuperacao.`
