...
- `guia_path`: `docs/planificacao/guias-bk/MF8/BK-MF8-02.md`
- `last_updated`: `2026-04-13`

## O que vamos fazer neste BK
Implementar e documentar o requisito `RNF31` associado a `Endpoint de health-check.`, com entrega verificavel e rastreabilidade 1:1 entre backlog, matriz e guia.
...
- Impacto funcional: `Endpoint de health-check.` desbloqueia valor direto no produto OPSA.
- Impacto tecnico: reforca a dimensao `CI/CD` sob o criterio `Pipeline, quality gates, deploy, rollback e health checks definidos e verificaveis.`.
...
- Onde tocar no projeto: Pipeline de integracao/entrega com gate, deploy controlado, rollback e health-check operacional.
...
- O requisito `RNF31` e demonstrado em cenario real de utilizacao.
...
1. Ler `RF/RNF` do BK (RNF31) e confirmar contexto tecnico antes de tocar no codigo.
...
4. Mapear onde alterar no projeto (frontend/backend/dados/integracao) para o BK `BK-MF8-02`.
5. Configurar pipeline com quality gate, deploy em ambiente alvo, health-check pos-deploy e rollback testado.
...
### Negativos
- [ ] Falha de testes bloqueia merge/deploy no quality gate.
- [ ] Health-check falhado apos deploy aciona rollback automatico/assistido.
- [ ] Artefacto invalido e rejeitado antes de promover para ambiente seguinte.
...
## Criterios de aceite
- `Pipeline executa quality gates obrigatorios antes de promover artefactos.`
- `Deploy inclui health-check objetivo e criterio de sucesso/reprovacao.`
- `Rollback esta definido e testado em cenario de falha controlada.`
