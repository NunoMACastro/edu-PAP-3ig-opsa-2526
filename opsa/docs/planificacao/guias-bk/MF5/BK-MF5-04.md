# BK-MF5-04 - Gerir linhas de credito (limites, utilizacao, alertas) por banco.
...
- `guia_path`: `docs/planificacao/guias-bk/MF5/BK-MF5-04.md`
- `last_updated`: `2026-04-13`

## O que vamos fazer neste BK
Implementar e documentar o requisito `RF64` associado a `Gerir linhas de credito (limites, utilizacao, alertas) por banco.`, com entrega verificavel e rastreabilidade 1:1 entre backlog, matriz e guia.
...
- Impacto funcional: `Gerir linhas de credito (limites, utilizacao, alertas) por banco.` desbloqueia valor direto no produto OPSA.
...
- O requisito `RF64` e demonstrado em cenario real de utilizacao.
...
- Dependencias tecnicas declaradas: `BK-MF3-02`.
...
1. Ler `RF/RNF` do BK (RF64) e confirmar contexto tecnico antes de tocar no codigo.
2. Validar dependencias declaradas (BK-MF3-02) e confirmar pre-condicoes em ambiente local.
3. Escrever mini-plano de implementacao (entrada, processamento, saida, erros) em 5-8 linhas.
4. Mapear onde alterar no projeto (frontend/backend/dados/integracao) para o BK `BK-MF5-04`.
