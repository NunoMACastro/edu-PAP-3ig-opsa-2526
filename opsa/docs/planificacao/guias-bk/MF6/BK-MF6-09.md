# BK-MF6-09 - Prevenir ataques (SQL/NoSQL Injection, XSS, CSRF, brute force).
...
- `guia_path`: `docs/planificacao/guias-bk/MF6/BK-MF6-09.md`
- `last_updated`: `2026-04-13`

## O que vamos fazer neste BK
Implementar e documentar o requisito `RNF16` associado a `Prevenir ataques (SQL/NoSQL Injection, XSS, CSRF, brute force).`, com entrega verificavel e rastreabilidade 1:1 entre backlog, matriz e guia.
...
- Impacto funcional: `Prevenir ataques (SQL/NoSQL Injection, XSS, CSRF, brute force).` desbloqueia valor direto no produto OPSA.
- Impacto tecnico: reforca a dimensao `Seguranca & Privacidade` sob o criterio `Controles de seguranca, RGPD/privacidade e protecoes de risco especificados.`.
...
- Onde tocar no projeto: Controles de seguranca aplicacional, protecao de dados e conformidade de privacidade.
...
- O requisito `RNF16` e demonstrado em cenario real de utilizacao.
...
1. Ler `RF/RNF` do BK (RNF16) e confirmar contexto tecnico antes de tocar no codigo.
...
4. Mapear onde alterar no projeto (frontend/backend/dados/integracao) para o BK `BK-MF6-09`.
5. Aplicar controlos de seguranca (auth, permissao, sanitizacao) e regras de minimizacao de dados.
...
### Negativos
- [ ] Token/sessao invalida e rejeitada sem exposicao de dados sensiveis.
- [ ] Tentativa de injecao/XSS e neutralizada com resposta controlada.
- [ ] Log com dado pessoal excessivo e bloqueado/mascarado pela politica.
...
## Criterios de aceite
- `Controlos de autenticacao/autorizacao validam acessos esperados e negados.`
- `Dados sensiveis seguem politica de protecao/mascaramento definida.`
- `Cenarios de ataque relevantes sao mitigados e evidenciados.`
