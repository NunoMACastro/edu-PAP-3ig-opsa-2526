# BK-MF2-02 - Criar e gerir **fornecedores**.
...
Neste BK vamos entregar `Criar e gerir **fornecedores**.` com resultado observável e verificável em review, alinhado com `RF10` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Criar e gerir **fornecedores**.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF10`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-03`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar e gerir **fornecedores**.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-03`.
