# BK-MF2-03 - Criar **artigos/serviços** (SKU, custo, preço, IVA).
...
Neste BK vamos entregar `Criar **artigos/serviços** (SKU, custo, preço, IVA).` com resultado observável e verificável em review, alinhado com `RF11` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Criar **artigos/serviços** (SKU, custo, preço, IVA).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF11`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-04`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar **artigos/serviços** (SKU, custo, preço, IVA).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-04`.
