...
## Contexto do BK
- Entrega alvo: implementar `Movimentos de stock: entradas, saídas, transferências, devoluções.` com rastreabilidade direta ao requisito `RF24`.
- Foco tecnico da macro: stock, analitica e contabilidade operacional.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Movimentos de stock: entradas, saídas, transferências, devoluções.` com autonomia técnica, garantindo cobertura do requisito `RF24` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF2-02` e o requisito `RF24`.
2. Validar dependencias técnicas (`BK-MF0-11, BK-MF0-12`) e preparar dados de teste mínimos para `Movimentos de stock: entradas, saídas, transferências, devoluções.`.
...
Contexto de rastreabilidade: `BK-MF2-02` -> `RF24`.
...
BEGIN
  ...
  IF q < 0 THEN
    RAISE EXCEPTION 'Saldo negativo nao permitido no BK BK-MF2-02';
  END IF;
END $$;
