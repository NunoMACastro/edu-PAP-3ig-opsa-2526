# BK-MF2-01 - Criar e gerir **clientes**.
...
Neste BK vamos entregar `Criar e gerir **clientes**.` com resultado observável e verificável em review, alinhado com `RF09` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Criar e gerir **clientes**.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF09`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-02`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar e gerir **clientes**.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-02`.
