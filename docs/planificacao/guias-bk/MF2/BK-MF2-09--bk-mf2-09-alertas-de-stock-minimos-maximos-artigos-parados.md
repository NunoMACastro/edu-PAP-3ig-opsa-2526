# BK-MF2-09 - Alertas de stock (mínimos, máximos, artigos parados).
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF2-06`
- `rf_rnf`: `RF30`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Alertas de stock (mínimos, máximos, artigos parados).` com resultado observável e verificável em review, alinhado com `RF30` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF2` e respeita as dependências `BK-MF2-06`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Alertas de stock (mínimos, máximos, artigos parados).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF30`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-10`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Alertas de stock (mínimos, máximos, artigos parados).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-10`.
