# BK-MF2-12 - Relatórios e filtros por centro de custo/segmento.
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF2-11`
- `rf_rnf`: `RF33`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Relatórios e filtros por centro de custo/segmento.` com resultado observável e verificável em review, alinhado com `RF33` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF2` e respeita as dependências `BK-MF2-11`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Relatórios e filtros por centro de custo/segmento.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF33`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-01`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Relatórios e filtros por centro de custo/segmento.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-01`.
