# BK-MF2-05 - Configurar **tabelas de IVA** (taxas, isenções, códigos).
...
Neste BK vamos entregar `Configurar **tabelas de IVA** (taxas, isenções, códigos).` com resultado observável e verificável em review, alinhado com `RF13` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF2` e respeita as dependências `-`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Configurar **tabelas de IVA** (taxas, isenções, códigos).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF13`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-06`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Configurar **tabelas de IVA** (taxas, isenções, códigos).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-06`.
