...
## Contexto do BK
- Entrega alvo: implementar `Alertas de stock (mínimos, máximos, artigos parados).` com rastreabilidade direta ao requisito `RF27`.
- Foco tecnico da macro: stock, analitica e contabilidade operacional.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Alertas de stock (mínimos, máximos, artigos parados).` com autonomia técnica, garantindo cobertura do requisito `RF27` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF2-05` e o requisito `RF27`.
2. Validar dependencias técnicas (`BK-MF2-02`) e preparar dados de teste mínimos para `Alertas de stock (mínimos, máximos, artigos parados).`.
...
Contexto de rastreabilidade: `BK-MF2-05` -> `RF27`.
...
BEGIN
  ...
  IF q < 0 THEN
    RAISE EXCEPTION 'Saldo negativo nao permitido no BK BK-MF2-05';
  END IF;
END $$;
