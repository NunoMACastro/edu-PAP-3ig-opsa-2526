# BK-MF2-04 - Criar **armazéns e localizações**.
...
Neste BK vamos entregar `Criar **armazéns e localizações**.` com resultado observável e verificável em review, alinhado com `RF12` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Criar **armazéns e localizações**.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF12`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-05`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar **armazéns e localizações**.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-05`.
