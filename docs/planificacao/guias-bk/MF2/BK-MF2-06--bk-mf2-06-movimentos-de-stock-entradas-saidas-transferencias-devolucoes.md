# BK-MF2-06 - Movimentos de stock: entradas, saídas, transferências, devoluções.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF2-03, BK-MF2-04`
- `rf_rnf`: `RF27`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Movimentos de stock: entradas, saídas, transferências, devoluções.` com resultado observável e verificável em review, alinhado com `RF27` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF2` e respeita as dependências `BK-MF2-03, BK-MF2-04`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Movimentos de stock: entradas, saídas, transferências, devoluções.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF27`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-07`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Movimentos de stock: entradas, saídas, transferências, devoluções.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-07`.
...
## Glossário rápido
- `catálogo`: conceito operativo central para este BK.
- `stock`: restrição técnica a garantir durante execução.
- `FIFO`: entidade funcional principal deste domínio.
- `armazém`: critério de controlo e validação desta entrega.
- `centro de custo`: evidência esperada para handoff e defesa.
