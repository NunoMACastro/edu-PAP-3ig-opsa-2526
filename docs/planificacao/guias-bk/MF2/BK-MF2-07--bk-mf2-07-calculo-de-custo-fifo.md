# BK-MF2-07 - Cálculo de custo (FIFO).
...
Neste BK vamos entregar `Cálculo de custo (FIFO).` com resultado observável e verificável em review, alinhado com `RF28` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Cálculo de custo (FIFO).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF28`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Cálculo de custo (FIFO).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-08`.
