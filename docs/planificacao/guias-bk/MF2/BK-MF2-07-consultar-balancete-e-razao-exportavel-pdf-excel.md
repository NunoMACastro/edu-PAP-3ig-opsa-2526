...
## Contexto do BK
- Entrega alvo: implementar `Consultar balancete e razão exportável (PDF/Excel).` com rastreabilidade direta ao requisito `RF29`.
- Foco tecnico da macro: stock, analitica e contabilidade operacional.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Consultar balancete e razão exportável (PDF/Excel).` com autonomia técnica, garantindo cobertura do requisito `RF29` e evidência objetiva para avaliação.
...
- Validar dependencias declaradas: `BK-MF2-06`.
...
- [ ] Sei mostrar onde esta o requisito `RF29` no sistema.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF2-07` e o requisito `RF29`.
2. Validar dependencias técnicas (`BK-MF2-06`) e preparar dados de teste mínimos para `Consultar balancete e razão exportável (PDF/Excel).`.
...
Contexto de rastreabilidade: `BK-MF2-07` -> `RF29`.
...
export function validarCabecalhoImportacao(headers: string[]) {
  ...
  if (faltam.length) {
    throw new Error(`BK BK-MF2-07: cabecalho invalido, faltam ${faltam.join(', ')}`);
  }
  return { ok: true, bk: 'BK-MF2-07' };
}
```
