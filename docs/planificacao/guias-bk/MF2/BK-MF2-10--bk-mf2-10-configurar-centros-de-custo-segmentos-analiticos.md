# BK-MF2-10 - Configurar **centros de custo** / segmentos analíticos.
...
Neste BK vamos entregar `Configurar **centros de custo** / segmentos analíticos.` com resultado observável e verificável em review, alinhado com `RF31` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF2` e respeita as dependências `BK-MF1-07`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Configurar **centros de custo** / segmentos analíticos.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF2` com rastreabilidade para `RF31`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-11`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Configurar **centros de custo** / segmentos analíticos.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-11`.
