...
## Bloco pedagogico
### Objetivo
Executar `Configurar centros de custo / segmentos analíticos.` com rastreabilidade explicita para `RF31` e demonstracao tecnica no contexto da sprint `S05-S06`.
...
3. Definir contrato de entrada/saida do fluxo principal para `Configurar centros de custo / segmentos analíticos.`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF2-07: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bk: 'BK-MF2-07', payload };
}
```
