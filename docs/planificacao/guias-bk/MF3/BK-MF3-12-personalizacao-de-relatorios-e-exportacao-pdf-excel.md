...
## Contexto do BK
- Entrega alvo: implementar `Personalização de relatórios e exportação (PDF/Excel).` com rastreabilidade direta ao requisito `RF48`.
- Foco tecnico da macro: tesouraria, integracoes e relatorio funcional.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Personalização de relatórios e exportação (PDF/Excel).` com rastreabilidade explicita para `RF48` e demonstracao tecnica no contexto da sprint `S07-S08`.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do BK-MF3-12 e o requisito `RF48`.
2. Verificar pre-condicoes tecnicas (BK-MF3-10) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Personalização de relatórios e exportação (PDF/Excel).`.
