...
## Contexto do BK
- Entrega alvo: implementar `Importar CSV/Excel de clientes, fornecedores, artigos e extratos.` com rastreabilidade direta ao requisito `RF35`.
- Foco tecnico da macro: tesouraria, integracoes e relatorio funcional.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Importar CSV/Excel de clientes, fornecedores, artigos e extratos.` com autonomia técnica, garantindo cobertura do requisito `RF35` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF3-05` e o requisito `RF35`.
...
Contexto de rastreabilidade: `BK-MF3-05` -> `RF35`.
...
export function sugerirCorrespondencias(extrato: Movimento[], pendentes: Movimento[]) {
  return extrato.map((m) => {
    ...
    return { bk: 'BK-MF3-05', movimento: m.id, documento: match?.id ?? null };
  });
}
```
