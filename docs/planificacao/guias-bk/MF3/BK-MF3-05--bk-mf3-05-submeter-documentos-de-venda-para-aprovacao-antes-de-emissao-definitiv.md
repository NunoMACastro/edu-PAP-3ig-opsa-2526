# BK-MF3-05 - Submeter documentos de venda para **aprovação** antes de emissão definitiva.
...
Neste BK vamos entregar `Submeter documentos de venda para **aprovação** antes de emissão definitiva.` com resultado observável e verificável em review, alinhado com `RF18` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF3` e respeita as dependências `BK-MF3-01`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Submeter documentos de venda para **aprovação** antes de emissão definitiva.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF18`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-06`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Submeter documentos de venda para **aprovação** antes de emissão definitiva.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-06`.
