# BK-MF3-11 - Aprovação de compras com estados “Rascunho → Aprovado → Lançado”.
...
Neste BK vamos entregar `Aprovação de compras com estados “Rascunho → Aprovado → Lançado”.` com resultado observável e verificável em review, alinhado com `RF24` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF3` e respeita as dependências `-`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Aprovação de compras com estados “Rascunho → Aprovado → Lançado”.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF24`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-12`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Aprovação de compras com estados “Rascunho → Aprovado → Lançado”.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-12`.
