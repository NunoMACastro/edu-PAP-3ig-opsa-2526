# BK-MF3-12 - Configurar **limites e papéis** para aprovações (por fornecedor/valor).
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF3-11`
- `rf_rnf`: `RF25`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Configurar **limites e papéis** para aprovações (por fornecedor/valor).` com resultado observável e verificável em review, alinhado com `RF25` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF3` e respeita as dependências `BK-MF3-11`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Configurar **limites e papéis** para aprovações (por fornecedor/valor).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF25`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-13`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Configurar **limites e papéis** para aprovações (por fornecedor/valor).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-13`.
