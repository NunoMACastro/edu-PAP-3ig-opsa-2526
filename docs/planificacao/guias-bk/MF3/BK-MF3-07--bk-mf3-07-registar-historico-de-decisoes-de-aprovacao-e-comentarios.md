# BK-MF3-07 - Registar histórico de **decisões de aprovação** e comentários.
...
Neste BK vamos entregar `Registar histórico de **decisões de aprovação** e comentários.` com resultado observável e verificável em review, alinhado com `RF20` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Registar histórico de **decisões de aprovação** e comentários.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF20`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Registar histórico de **decisões de aprovação** e comentários.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-08`.
