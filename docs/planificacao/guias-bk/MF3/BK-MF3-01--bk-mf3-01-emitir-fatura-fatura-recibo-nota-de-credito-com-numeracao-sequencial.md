# BK-MF3-01 - Emitir **Fatura, Fatura-Recibo, Nota de Crédito**, com numeração sequencial.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF2-01, BK-MF2-03, BK-MF2-05`
- `rf_rnf`: `RF14`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Emitir **Fatura, Fatura-Recibo, Nota de Crédito**, com numeração sequencial.` com resultado observável e verificável em review, alinhado com `RF14` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF3` e respeita as dependências `BK-MF2-01, BK-MF2-03, BK-MF2-05`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Emitir **Fatura, Fatura-Recibo, Nota de Crédito**, com numeração sequencial.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF14`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-02`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Emitir **Fatura, Fatura-Recibo, Nota de Crédito**, com numeração sequencial.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-02`.
...
## Glossário rápido
- `faturação`: conceito operativo central para este BK.
- `compra`: restrição técnica a garantir durante execução.
- `aprovação`: entidade funcional principal deste domínio.
- `IVA`: critério de controlo e validação desta entrega.
- `lançamento automático`: evidência esperada para handoff e defesa.
