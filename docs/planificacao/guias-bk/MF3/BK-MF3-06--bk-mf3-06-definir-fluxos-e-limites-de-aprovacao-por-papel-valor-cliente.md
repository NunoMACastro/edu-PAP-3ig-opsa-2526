# BK-MF3-06 - Definir **fluxos e limites** de aprovação (por papel, valor, cliente).
...
Neste BK vamos entregar `Definir **fluxos e limites** de aprovação (por papel, valor, cliente).` com resultado observável e verificável em review, alinhado com `RF19` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF3` e respeita as dependências `BK-MF3-05`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Definir **fluxos e limites** de aprovação (por papel, valor, cliente).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF19`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-07`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Definir **fluxos e limites** de aprovação (por papel, valor, cliente).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-07`.
