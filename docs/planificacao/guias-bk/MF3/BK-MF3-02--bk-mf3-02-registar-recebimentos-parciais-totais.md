# BK-MF3-02 - Registar **recebimentos** (parciais/totais).
...
Neste BK vamos entregar `Registar **recebimentos** (parciais/totais).` com resultado observável e verificável em review, alinhado com `RF15` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Registar **recebimentos** (parciais/totais).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF15`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-03`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Registar **recebimentos** (parciais/totais).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-03`.
