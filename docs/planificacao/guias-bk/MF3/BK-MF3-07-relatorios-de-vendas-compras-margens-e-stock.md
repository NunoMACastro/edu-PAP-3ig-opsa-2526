...
## Contexto do BK
- Entrega alvo: implementar `Relatórios de vendas, compras, margens e stock.` com rastreabilidade direta ao requisito `RF37`.
- Foco tecnico da macro: tesouraria, integracoes e relatorio funcional.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Relatórios de vendas, compras, margens e stock.` com autonomia técnica, garantindo cobertura do requisito `RF37` e evidência objetiva para avaliação.
...
- Validar dependencias declaradas: `BK-MF1-02, BK-MF1-07, BK-MF2-02`.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF3-07` e o requisito `RF37`.
2. Validar dependencias técnicas (`BK-MF1-02, BK-MF1-07, BK-MF2-02`) e preparar dados de teste mínimos para `Relatórios de vendas, compras, margens e stock.`.
...
Contexto de rastreabilidade: `BK-MF3-07` -> `RF37`.

```ts
type LinhaDocumento = { conta: string; base: number; taxaIVA: number };

export function validarDocumentoFiscal(linhas: LinhaDocumento[], bkId = 'BK-MF3-07') {
