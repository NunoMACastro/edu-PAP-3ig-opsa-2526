# BK-MF3-09 - Registar **pagamentos** (parciais/totais).
...
Neste BK vamos entregar `Registar **pagamentos** (parciais/totais).` com resultado observável e verificável em review, alinhado com `RF22` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Registar **pagamentos** (parciais/totais).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF22`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-10`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Registar **pagamentos** (parciais/totais).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-10`.
