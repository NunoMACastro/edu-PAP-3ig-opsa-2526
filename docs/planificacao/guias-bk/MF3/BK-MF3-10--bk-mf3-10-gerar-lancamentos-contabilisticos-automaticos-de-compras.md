# BK-MF3-10 - Gerar **lançamentos contabilísticos automáticos** de compras.
...
Neste BK vamos entregar `Gerar **lançamentos contabilísticos automáticos** de compras.` com resultado observável e verificável em review, alinhado com `RF23` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF3` e respeita as dependências `BK-MF3-08`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Gerar **lançamentos contabilísticos automáticos** de compras.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF3` com rastreabilidade para `RF23`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-11`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Gerar **lançamentos contabilísticos automáticos** de compras.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-11`.
