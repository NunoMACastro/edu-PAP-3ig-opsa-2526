# BK-MF0-01 - Dashboard e listagens devem carregar em ≤ 2 segundos.
...
Neste BK vamos entregar `Dashboard e listagens devem carregar em ≤ 2 segundos.` com resultado observável e verificável em review, alinhado com `RNF07` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Dashboard e listagens devem carregar em ≤ 2 segundos.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF07`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-02`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Dashboard e listagens devem carregar em ≤ 2 segundos.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-02`.
