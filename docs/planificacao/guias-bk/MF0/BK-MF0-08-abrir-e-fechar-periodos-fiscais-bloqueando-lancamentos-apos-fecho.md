...
## Bloco pedagogico
### Objetivo
Executar `Abrir e fechar períodos fiscais, bloqueando lançamentos após fecho.` com autonomia técnica, garantindo cobertura do requisito `RF08` e evidência objetiva para avaliação.
...
2. Validar dependencias técnicas (`BK-MF0-07`) e preparar dados de teste mínimos para `Abrir e fechar períodos fiscais, bloqueando lançamentos após fecho.`.
...
export function validarComando(ctx: Contexto, role: string) {
  ...
  if (!ctx.roles.includes(role)) throw new Error(`Permissao insuficiente para RF08`);
  return { bkId: 'BK-MF0-08', ok: true };
}
```
