...
0. **Objetivo (~10 min): Confirmar contrato e preparar ambiente**
   - Descricao detalhada do objetivo: Rever o requisito RF07, a linha do BK-MF0-07 no backlog/matriz e confirmar que o repositorio ainda esta em modo sem codigo real fora do mockup.
   ...
   - Snippet de referencia: `BK-MF0-07 -> RF07 -> GET /api/accounting/accounts, POST /api/accounting/accounts, POST /api/accounting/accounts/import`
...
1. **Objetivo (~25 min): Modelar dados e constraints**
   - Descricao detalhada do objetivo: Criar ou ajustar o schema para Account, sempre com isolamento por empresa quando o dado pertencer a uma empresa.
   ...
   - Snippet de referencia: `model Account { /* BK-MF0-07 */ }`
...
2. **Objetivo (~25 min): Criar validadores backend**
   ...
   - Snippet de referencia: `validateAccount(req.body)`
   - O que verificar: Payload invalido devolve 400 com mensagem acionavel.

3. **Objetivo (~25 min): Implementar service de negocio**
   - Descricao detalhada do objetivo: Criar o service que executa a regra principal do BK-MF0-07.
...
4. **Objetivo (~25 min): Expor rotas e controller**
   - Descricao detalhada do objetivo: Criar endpoints: GET /api/accounting/accounts, POST /api/accounting/accounts, POST /api/accounting/accounts/import.
...
5. **Objetivo (~25 min): Ligar frontend ao mockup**
   - Descricao detalhada do objetivo: Transformar a referencia visual do mockup em fluxo real sem exigir pixel-perfect. Referencia UI: mockup/src/app/components/modules/Contabilidade.tsx, filtros por Conta e tabelas contabilisticas.
   ...
   - Ficheiro a rever: mockup/src/app/components/modules/Contabilidade.tsx, filtros por Conta e tabelas contabilisticas
   - Ficheiro alvo: apps/web/src/modules/contabilidade/; apps/web/src/lib/apiClient.ts
   - Snippet de referencia: `apiClient.request('GET /api/accounting/accounts')`
...
6. **Objetivo (~25 min): Executar smoke e negativos**
   - Descricao detalhada do objetivo: Validar o caso principal e os cenarios negativos minimos do BK-MF0-07.
   ...
   - Ficheiro alvo: apps/api/tests/BK-MF0-07.test.js; docs/evidence/BK-MF0-07.md
   - Snippet de referencia: `curl -i http://localhost:3000/api/accounting/accounts`
   - O que verificar: Smoke aprovado e pelo menos 3 negativos registados.

7. **Objetivo (~15 min): Preparar handoff e evidence**
   - Descricao detalhada do objetivo: Registar o que ficou construido, os ficheiros alterados, comandos e riscos para o proximo BK (BK-MF0-08).
   ...
   - Ficheiro alvo: docs/evidence/BK-MF0-07.md ou descricao do PR
   - Snippet de referencia: `handoff.next = 'BK-MF0-08'`
   - O que verificar: Handoff referencia contratos reutilizaveis e riscos abertos.

#### Checklist de validacao (DERIVADO):

- Smoke: executar o fluxo principal de `GET /api/accounting/accounts` e confirmar resultado funcional na UI/API.
...
- UI/mockup: comparar nomes e fluxo com `mockup/src/app/components/modules/Contabilidade.tsx, filtros por Conta e tabelas contabilisticas`, sem exigir pixel-perfect.
...
| --- | --- | --- | --- | --- |
| N1 | 6 | Importar CSV com codigo ABC. | Linha rejeitada com erro explicito. | Evita contas fora do formato contabilistico. |
| N2 | 6 | Criar conta duplicada na mesma empresa. | Resposta 409. | Evita ambiguidade nos lancamentos. |
| N3 | 6 | Importar ficheiro vazio. | Resposta 400 com mensagem clara. | Evita importacao sem efeito. |

#### Criterios de aceite:

- Outputs:
- Modelo Account com companyId e code unico por empresa.
- Endpoint de listagem/criacao/importacao.
- Validador de CSV com erros por linha.
- UI ou placeholder controlado para consultar/importar contas.
- Handoff para BK-MF0-08 e BK-MF2-06.
- Verificacoes:
- Caso valido de `GET /api/accounting/accounts` concluido com codigo HTTP adequado.
...
- Continuidade:
- O BK seguinte `BK-MF0-08` consegue reutilizar os contratos deste BK.
...
- TODO (BLOCKER): confirmar fonte oficial/ficheiro base do plano SNC a importar.
- FOLLOW-UP: BK-MF0-08 deve bloquear alteracoes contabilisticas em periodos fechados.

## Bloco pedagogico

### Objetivo

Construir Criar/importar plano de contas (SNC) de forma executavel, segura e rastreavel ao RF07.

### Pre-requisitos

- Ler RF07, este guia e os documentos canonicos indicados na pre-leitura.
...
- [ ] Sei explicar que parte da app fica pronta no BK-MF0-07.
...
- Requisito: `RF07`
- Dependencias: `-`
- Endpoint(s): `GET /api/accounting/accounts, POST /api/accounting/accounts, POST /api/accounting/accounts/import`
...
Contexto de rastreabilidade: `BK-MF0-07` -> `RF07`.
```js
export function validateAccount(row) {
  if (!/^\d{2,8}$/.test(row.code)) throw new Error(`Codigo invalido: ${row.code}`);
  if (!row.name?.trim()) throw new Error("Nome da conta e obrigatorio");
  return { code: row.code, name: row.name.trim(), active: row.active !== false };
}
```
