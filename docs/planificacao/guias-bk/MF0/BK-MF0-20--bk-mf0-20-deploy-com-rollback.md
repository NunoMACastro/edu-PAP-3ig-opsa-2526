# BK-MF0-20 - Deploy com rollback.

## Header
- `doc_id`: `GUIA-BK-MF0-20`
- `bk_id`: `BK-MF0-20`
- `macro`: `MF0`
- `owner`: `Oleksii`
- `apoio`: `Sofia`
...
Neste BK vamos entregar `Deploy com rollback.` com resultado observável e verificável em review, alinhado com `RNF32` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Deploy com rollback.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF32`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-21`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Deploy com rollback.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-21`.
