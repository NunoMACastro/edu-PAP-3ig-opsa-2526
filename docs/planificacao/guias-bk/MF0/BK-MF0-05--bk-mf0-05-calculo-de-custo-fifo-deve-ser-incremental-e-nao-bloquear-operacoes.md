# BK-MF0-05 - Cálculo de custo (FIFO) deve ser incremental e não bloquear operações.
...
Neste BK vamos entregar `Cálculo de custo (FIFO) deve ser incremental e não bloquear operações.` com resultado observável e verificável em review, alinhado com `RNF11` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Cálculo de custo (FIFO) deve ser incremental e não bloquear operações.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF11`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-06`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Cálculo de custo (FIFO) deve ser incremental e não bloquear operações.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-06`.
