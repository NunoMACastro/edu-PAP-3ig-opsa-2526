# BK-MF0-04 - Reconciliação bancária deve sugerir correspondências em ≤ 3 segundos.
...
Neste BK vamos entregar `Reconciliação bancária deve sugerir correspondências em ≤ 3 segundos.` com resultado observável e verificável em review, alinhado com `RNF10` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Reconciliação bancária deve sugerir correspondências em ≤ 3 segundos.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF10`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-05`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Reconciliação bancária deve sugerir correspondências em ≤ 3 segundos.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-05`.
