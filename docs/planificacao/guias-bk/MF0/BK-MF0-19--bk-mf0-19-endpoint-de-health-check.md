# BK-MF0-19 - Endpoint de health-check.
...
Neste BK vamos entregar `Endpoint de health-check.` com resultado observável e verificável em review, alinhado com `RNF31` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Endpoint de health-check.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF31`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-20`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Endpoint de health-check.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-20`.
