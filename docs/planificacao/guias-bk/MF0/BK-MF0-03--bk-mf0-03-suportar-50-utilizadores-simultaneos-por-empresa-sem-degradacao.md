# BK-MF0-03 - Suportar ≥ 50 utilizadores simultâneos por empresa sem degradação.
...
Neste BK vamos entregar `Suportar ≥ 50 utilizadores simultâneos por empresa sem degradação.` com resultado observável e verificável em review, alinhado com `RNF09` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Suportar ≥ 50 utilizadores simultâneos por empresa sem degradação.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF09`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-04`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Suportar ≥ 50 utilizadores simultâneos por empresa sem degradação.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-04`.
