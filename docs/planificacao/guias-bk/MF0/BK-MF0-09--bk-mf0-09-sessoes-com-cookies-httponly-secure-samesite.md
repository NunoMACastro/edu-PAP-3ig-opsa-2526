# BK-MF0-09 - Sessões com cookies HttpOnly + Secure + SameSite.
...
Neste BK vamos entregar `Sessões com cookies HttpOnly + Secure + SameSite.` com resultado observável e verificável em review, alinhado com `RNF15` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Sessões com cookies HttpOnly + Secure + SameSite.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF15`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-10`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Sessões com cookies HttpOnly + Secure + SameSite.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-10`.
