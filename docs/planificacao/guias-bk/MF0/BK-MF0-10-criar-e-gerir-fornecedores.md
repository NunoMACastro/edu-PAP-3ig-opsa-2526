...
## Contexto do BK
- Entrega alvo: implementar `Criar e gerir fornecedores.` com rastreabilidade direta ao requisito `RF10`.
- Foco tecnico da macro: fundacoes de autenticacao, perfis, empresa e dados mestre.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Criar e gerir fornecedores.` com autonomia técnica, garantindo cobertura do requisito `RF10` e evidência objetiva para avaliação.
- Intenção pedagógica da macro `MF0`: Instalar base segura de identidade e dados mestre para desbloquear todo o ERP..

### Pre-requisitos
- Ler o requisito `RF10` e rever o contexto em `MATRIZ-CANONICA-BK.md` e `BACKLOG-MVP.md`.
...
- [ ] Sei mostrar onde esta o requisito `RF10` no sistema.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF0-10` e o requisito `RF10`.
2. Validar dependencias técnicas (`-`) e preparar dados de teste mínimos para `Criar e gerir fornecedores.`.
...
export function validarComando(ctx: Contexto, role: string) {
  ...
  if (!ctx.roles.includes(role)) throw new Error(`Permissao insuficiente para RF10`);
  return { bkId: 'BK-MF0-10', ok: true };
}
```
