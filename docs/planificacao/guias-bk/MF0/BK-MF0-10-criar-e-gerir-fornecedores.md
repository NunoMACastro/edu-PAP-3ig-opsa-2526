...
## Bloco pedagogico
### Objetivo
Executar `Criar e gerir fornecedores.` com rastreabilidade explicita para `RF10` e demonstracao tecnica no contexto da sprint `S01-S02`.
...
3. Definir contrato de entrada/saida do fluxo principal para `Criar e gerir fornecedores.`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF0-10: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bk: 'BK-MF0-10', payload };
}
```
