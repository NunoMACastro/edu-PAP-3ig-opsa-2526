...
0. **Objetivo (~10 min): Confirmar contrato e preparar ambiente**
   - Descricao detalhada do objetivo: Rever o requisito RF05, a linha do BK-MF0-05 no backlog/matriz e confirmar que o repositorio ainda esta em modo sem codigo real fora do mockup.
   ...
   - Snippet de referencia: `BK-MF0-05 -> RF05 -> POST /api/auth/password/forgot, POST /api/auth/password/reset`
...
1. **Objetivo (~25 min): Modelar dados e constraints**
   - Descricao detalhada do objetivo: Criar ou ajustar o schema para PasswordResetToken, sempre com isolamento por empresa quando o dado pertencer a uma empresa.
   ...
   - Snippet de referencia: `model PasswordResetToken { /* BK-MF0-05 */ }`
...
2. **Objetivo (~25 min): Criar validadores backend**
   ...
   - Snippet de referencia: `validatePasswordResetToken(req.body)`
   - O que verificar: Payload invalido devolve 400 com mensagem acionavel.

3. **Objetivo (~25 min): Implementar service de negocio**
   - Descricao detalhada do objetivo: Criar o service que executa a regra principal do BK-MF0-05.
...
4. **Objetivo (~25 min): Expor rotas e controller**
   - Descricao detalhada do objetivo: Criar endpoints: POST /api/auth/password/forgot, POST /api/auth/password/reset.
...
5. **Objetivo (~25 min): Ligar frontend ao mockup**
   - Descricao detalhada do objetivo: Transformar a referencia visual do mockup em fluxo real sem exigir pixel-perfect. Referencia UI: mockup/src/app/components/Login.tsx, link Esqueceu a password?.
   ...
   - Ficheiro a rever: mockup/src/app/components/Login.tsx, link Esqueceu a password?
   - Ficheiro alvo: apps/web/src/modules/identidade/; apps/web/src/lib/apiClient.ts
   - Snippet de referencia: `apiClient.request('POST /api/auth/password/forgot')`
...
6. **Objetivo (~25 min): Executar smoke e negativos**
   - Descricao detalhada do objetivo: Validar o caso principal e os cenarios negativos minimos do BK-MF0-05.
   ...
   - Ficheiro alvo: apps/api/tests/BK-MF0-05.test.js; docs/evidence/BK-MF0-05.md
   - Snippet de referencia: `curl -i http://localhost:3000/api/auth/password/forgot`
   - O que verificar: Smoke aprovado e pelo menos 3 negativos registados.

7. **Objetivo (~15 min): Preparar handoff e evidence**
   - Descricao detalhada do objetivo: Registar o que ficou construido, os ficheiros alterados, comandos e riscos para o proximo BK (BK-MF0-06).
   ...
   - Ficheiro alvo: docs/evidence/BK-MF0-05.md ou descricao do PR
   - Snippet de referencia: `handoff.next = 'BK-MF0-06'`
   - O que verificar: Handoff referencia contratos reutilizaveis e riscos abertos.

#### Checklist de validacao (DERIVADO):

- Smoke: executar o fluxo principal de `POST /api/auth/password/forgot` e confirmar resultado funcional na UI/API.
...
- UI/mockup: comparar nomes e fluxo com `mockup/src/app/components/Login.tsx, link Esqueceu a password?`, sem exigir pixel-perfect.
...
| --- | --- | --- | --- | --- |
| N1 | 6 | Pedido com email inexistente. | Resposta 200 generica. | Evita user enumeration. |
| N2 | 6 | Reset com token expirado. | Resposta 400 e password inalterada. | Evita uso tardio de token. |
| N3 | 6 | Nova password fraca. | Resposta 400 com mensagem clara. | Evita credenciais triviais. |

#### Criterios de aceite:

- Outputs:
- Modelo PasswordResetToken com expiracao.
- Service de pedido e reset sem user enumeration.
- Formulario frontend para pedir link e outro para nova password.
- Teste negativo de token expirado/usado.
- Handoff para BK-MF0-06 sem alterar contrato de identidade.
- Verificacoes:
- Caso valido de `POST /api/auth/password/forgot` concluido com codigo HTTP adequado.
...
- Continuidade:
- O BK seguinte `BK-MF0-06` consegue reutilizar os contratos deste BK.
...
- TODO (BLOCKER): confirmar dependencia/servico para email no ambiente real.
- FOLLOW-UP: RNF21 reforcara integracao de email em fase posterior.

## Bloco pedagogico

### Objetivo

Construir Recuperação de password via email de forma executavel, segura e rastreavel ao RF05.

### Pre-requisitos

- Ler RF05, este guia e os documentos canonicos indicados na pre-leitura.
...
- [ ] Sei explicar que parte da app fica pronta no BK-MF0-05.
...
- Requisito: `RF05`
- Dependencias: `-`
- Endpoint(s): `POST /api/auth/password/forgot, POST /api/auth/password/reset`
...
Contexto de rastreabilidade: `BK-MF0-05` -> `RF05`.
```js
export async function requestPasswordReset(email) {
  const user = await users.findByEmail(email);
  if (!user) return { ok: true };
  const token = crypto.randomUUID();
  await resetTokens.create({ userId: user.id, tokenHash: hashToken(token), expiresMinutes: 30 });
