...
2. Verificar pre-condicoes tecnicas (BK-MF0-03) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Gestão de utilizadores: convite, remoção e definição de papéis.`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF0-04: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bk: 'BK-MF0-04', payload };
}
```
