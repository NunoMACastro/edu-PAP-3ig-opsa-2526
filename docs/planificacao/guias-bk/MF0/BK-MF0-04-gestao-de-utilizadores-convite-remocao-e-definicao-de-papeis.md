...
## Contexto do BK
- Entrega alvo: implementar `Gestão de utilizadores: convite, remoção e definição de papéis.` com rastreabilidade direta ao requisito `RF04`.
- Foco tecnico da macro: fundacoes de autenticacao, perfis, empresa e dados mestre.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Gestão de utilizadores: convite, remoção e definição de papéis.` com autonomia técnica, garantindo cobertura do requisito `RF04` e evidência objetiva para avaliação.
...
- Validar dependencias declaradas: `BK-MF0-03`.
...
- [ ] Sei mostrar onde esta o requisito `RF04` no sistema.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF0-04` e o requisito `RF04`.
2. Validar dependencias técnicas (`BK-MF0-03`) e preparar dados de teste mínimos para `Gestão de utilizadores: convite, remoção e definição de papéis.`.
...
export function validarComando(ctx: Contexto, role: string) {
  ...
  if (!ctx.roles.includes(role)) throw new Error(`Permissao insuficiente para RF04`);
  return { bkId: 'BK-MF0-04', ok: true };
}
```
