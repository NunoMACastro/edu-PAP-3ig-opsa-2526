# BK-MF0-12 - Auditoria obrigatória em operações sensíveis.
...
Neste BK vamos entregar `Auditoria obrigatória em operações sensíveis.` com resultado observável e verificável em review, alinhado com `RNF18` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Auditoria obrigatória em operações sensíveis.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF18`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-13`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Auditoria obrigatória em operações sensíveis.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-13`.
