# BK-MF0-17 - Testes automatizados para módulos críticos (faturação, IVA, balancetes, reconciliação).
...
Neste BK vamos entregar `Testes automatizados para módulos críticos (faturação, IVA, balancetes, reconciliação).` com resultado observável e verificável em review, alinhado com `RNF29` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF0` e respeita as dependências `-`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Testes automatizados para módulos críticos (faturação, IVA, balancetes, reconciliação).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF29`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-18`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Testes automatizados para módulos críticos (faturação, IVA, balancetes, reconciliação).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-18`.
...
## Glossário rápido
- `baseline técnico`: conceito operativo central para este BK.
- `hardening`: restrição técnica a garantir durante execução.
- `SLA`: entidade funcional principal deste domínio.
- `rollback`: critério de controlo e validação desta entrega.
- `observabilidade`: evidência esperada para handoff e defesa.
