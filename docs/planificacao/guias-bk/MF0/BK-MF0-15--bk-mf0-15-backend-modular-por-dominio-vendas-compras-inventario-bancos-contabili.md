# BK-MF0-15 - Backend modular por domínio (vendas, compras, inventário, bancos, contabilidade, IA).
...
Neste BK vamos entregar `Backend modular por domínio (vendas, compras, inventário, bancos, contabilidade, IA).` com resultado observável e verificável em review, alinhado com `RNF27` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Backend modular por domínio (vendas, compras, inventário, bancos, contabilidade, IA).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF27`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-16`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Backend modular por domínio (vendas, compras, inventário, bancos, contabilidade, IA).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-16`.
