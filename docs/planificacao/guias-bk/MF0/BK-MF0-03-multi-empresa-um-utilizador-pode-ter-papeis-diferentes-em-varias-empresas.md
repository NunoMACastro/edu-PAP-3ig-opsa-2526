...
## Bloco pedagogico
### Objetivo
Executar `Multi-empresa (um utilizador pode ter papéis diferentes em várias empresas).` com autonomia técnica, garantindo cobertura do requisito `RF03` e evidência objetiva para avaliação.
...
2. Validar dependencias técnicas (`BK-MF0-02`) e preparar dados de teste mínimos para `Multi-empresa (um utilizador pode ter papéis diferentes em várias empresas).`.
...
export function validarComando(ctx: Contexto, role: string) {
  ...
  if (!ctx.roles.includes(role)) throw new Error(`Permissao insuficiente para RF03`);
  return { bkId: 'BK-MF0-03', ok: true };
}
```
