# BK-MF0-07 - Toda a comunicação deve usar HTTPS (TLS 1.2+).
...
Neste BK vamos entregar `Toda a comunicação deve usar HTTPS (TLS 1.2+).` com resultado observável e verificável em review, alinhado com `RNF13` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Toda a comunicação deve usar HTTPS (TLS 1.2+).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF13`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Toda a comunicação deve usar HTTPS (TLS 1.2+).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-08`.
