...
## Bloco pedagogico
### Objetivo
Executar `Papéis e permissões (Admin, Gestor, Contabilista, Operacional, Auditor).` com rastreabilidade explicita para `RF02` e demonstracao tecnica no contexto da sprint `S01-S02`.
...
2. Verificar pre-condicoes tecnicas (BK-MF0-01) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Papéis e permissões (Admin, Gestor, Contabilista, Operacional, Auditor).`.
