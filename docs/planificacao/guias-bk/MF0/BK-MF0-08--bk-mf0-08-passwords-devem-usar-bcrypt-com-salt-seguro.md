# BK-MF0-08 - Passwords devem usar bcrypt com salt seguro.
...
Neste BK vamos entregar `Passwords devem usar bcrypt com salt seguro.` com resultado observável e verificável em review, alinhado com `RNF14` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Passwords devem usar bcrypt com salt seguro.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF14`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-09`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Passwords devem usar bcrypt com salt seguro.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-09`.
