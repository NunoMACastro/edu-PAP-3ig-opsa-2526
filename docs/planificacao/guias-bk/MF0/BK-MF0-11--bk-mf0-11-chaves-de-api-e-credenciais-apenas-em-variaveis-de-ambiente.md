# BK-MF0-11 - Chaves de API e credenciais apenas em variáveis de ambiente.
...
Neste BK vamos entregar `Chaves de API e credenciais apenas em variáveis de ambiente.` com resultado observável e verificável em review, alinhado com `RNF17` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Chaves de API e credenciais apenas em variáveis de ambiente.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF17`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-12`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Chaves de API e credenciais apenas em variáveis de ambiente.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-12`.
