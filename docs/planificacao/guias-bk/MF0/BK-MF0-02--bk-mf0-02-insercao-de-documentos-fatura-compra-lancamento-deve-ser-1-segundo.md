# BK-MF0-02 - Inserção de documentos (fatura, compra, lançamento) deve ser ≤ 1 segundo.
...
Neste BK vamos entregar `Inserção de documentos (fatura, compra, lançamento) deve ser ≤ 1 segundo.` com resultado observável e verificável em review, alinhado com `RNF08` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Inserção de documentos (fatura, compra, lançamento) deve ser ≤ 1 segundo.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF08`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-03`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Inserção de documentos (fatura, compra, lançamento) deve ser ≤ 1 segundo.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-03`.
