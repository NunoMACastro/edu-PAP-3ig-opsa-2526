# BK-MF0-18 - Logs estruturados (info, warn, error, audit).
...
Neste BK vamos entregar `Logs estruturados (info, warn, error, audit).` com resultado observável e verificável em review, alinhado com `RNF30` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Logs estruturados (info, warn, error, audit).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF0` com rastreabilidade para `RNF30`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-19`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Logs estruturados (info, warn, error, audit).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-19`.
