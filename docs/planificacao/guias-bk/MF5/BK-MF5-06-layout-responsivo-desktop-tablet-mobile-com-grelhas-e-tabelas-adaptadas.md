...
- `guia_path`: `docs/planificacao/guias-bk/MF5/BK-MF5-06-layout-responsivo-desktop-tablet-mobile-com-grelhas-e-tabelas-adaptadas.md`
- `last_updated`: `2026-04-17`

## Contexto do BK
- Entrega alvo: implementar `Layout responsivo (desktop > tablet > mobile) com grelhas e tabelas adaptadas.` com rastreabilidade direta ao requisito `RNF02`.
- Foco tecnico da macro: experiencia de utilizacao e fluxos transversais de negocio.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Layout responsivo (desktop > tablet > mobile) com grelhas e tabelas adaptadas.` com autonomia técnica, garantindo cobertura do requisito `RNF02` e evidência objetiva para avaliação.
...
- [ ] Sei mostrar onde esta o requisito `RNF02` no sistema.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF5-06` e o requisito `RNF02`.
2. Validar dependencias técnicas (`-`) e preparar dados de teste mínimos para `Layout responsivo (desktop > tablet > mobile) com grelhas e tabelas adaptadas.`.
...
export function validarResultado(res: Resultado) {
  ...
  return { bk: 'BK-MF5-06', validado: true };
}
```
