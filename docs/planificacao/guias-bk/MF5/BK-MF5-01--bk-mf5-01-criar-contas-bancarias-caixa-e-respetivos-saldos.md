# BK-MF5-01 - Criar **contas bancárias/caixa** e respetivos saldos.
...
Neste BK vamos entregar `Criar **contas bancárias/caixa** e respetivos saldos.` com resultado observável e verificável em review, alinhado com `RF38` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF5` e respeita as dependências `-`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Criar **contas bancárias/caixa** e respetivos saldos.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF5` com rastreabilidade para `RF38`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-02`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar **contas bancárias/caixa** e respetivos saldos.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-02`.
