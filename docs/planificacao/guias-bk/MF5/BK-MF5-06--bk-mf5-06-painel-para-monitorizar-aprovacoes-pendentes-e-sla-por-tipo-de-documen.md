# BK-MF5-06 - Painel para monitorizar aprovações pendentes e SLA por tipo de documento.
...
Neste BK vamos entregar `Painel para monitorizar aprovações pendentes e SLA por tipo de documento.` com resultado observável e verificável em review, alinhado com `RF62` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF5` e respeita as dependências `BK-MF5-04`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Painel para monitorizar aprovações pendentes e SLA por tipo de documento.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF5` com rastreabilidade para `RF62`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-07`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Painel para monitorizar aprovações pendentes e SLA por tipo de documento.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-07`.
