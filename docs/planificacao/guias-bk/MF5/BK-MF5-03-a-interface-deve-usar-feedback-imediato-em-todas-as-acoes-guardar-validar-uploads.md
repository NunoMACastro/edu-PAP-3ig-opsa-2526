...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF5-03` e o requisito `RNF03`.
...
Contexto de rastreabilidade: `BK-MF5-03` -> `RNF03`.
...
export function validarCabecalhoImportacao(headers: string[]) {
  ...
  if (faltam.length) {
    throw new Error(`BK BK-MF5-03: cabecalho invalido, faltam ${faltam.join(', ')}`);
  }
  return { ok: true, bk: 'BK-MF5-03' };
}
```
