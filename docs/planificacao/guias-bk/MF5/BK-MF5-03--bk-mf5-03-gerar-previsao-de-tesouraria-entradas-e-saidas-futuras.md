# BK-MF5-03 - Gerar **previsão de tesouraria** (entradas e saídas futuras).
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF3-02, BK-MF3-09`
- `rf_rnf`: `RF40`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Gerar **previsão de tesouraria** (entradas e saídas futuras).` com resultado observável e verificável em review, alinhado com `RF40` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF5` e respeita as dependências `BK-MF3-02, BK-MF3-09`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Gerar **previsão de tesouraria** (entradas e saídas futuras).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF5` com rastreabilidade para `RF40`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-04`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Gerar **previsão de tesouraria** (entradas e saídas futuras).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-04`.
