# BK-MF5-04 - Definir **workflows de aprovação** por documento (compra, venda, pagamento).
...
- `estado`: `TODO`
- `esforço`: `L`
- `dependências`: `BK-MF3-08, BK-MF2-09`
- `rf_rnf`: `RF60`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Definir **workflows de aprovação** por documento (compra, venda, pagamento).` com resultado observável e verificável em review, alinhado com `RF60` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF5` e respeita as dependências `BK-MF3-08, BK-MF2-09`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Definir **workflows de aprovação** por documento (compra, venda, pagamento).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF5` com rastreabilidade para `RF60`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-05`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Definir **workflows de aprovação** por documento (compra, venda, pagamento).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-05`.
