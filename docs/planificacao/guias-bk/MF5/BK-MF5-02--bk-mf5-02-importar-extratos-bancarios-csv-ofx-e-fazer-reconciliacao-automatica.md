# BK-MF5-02 - Importar **extratos bancários** (CSV/OFX) e fazer reconciliação automática.
...
- `estado`: `TODO`
- `esforço`: `L`
- `dependências`: `BK-MF5-01, BK-MF3-01, BK-MF3-08`
- `rf_rnf`: `RF39`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Importar **extratos bancários** (CSV/OFX) e fazer reconciliação automática.` com resultado observável e verificável em review, alinhado com `RF39` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF5` e respeita as dependências `BK-MF5-01, BK-MF3-01, BK-MF3-08`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Importar **extratos bancários** (CSV/OFX) e fazer reconciliação automática.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF5` com rastreabilidade para `RF39`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-03`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Importar **extratos bancários** (CSV/OFX) e fazer reconciliação automática.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-03`.
