# BK-MF5-07 - Agendar pagamentos/recebimentos futuros com integração às previsões de tesouraria.
...
Neste BK vamos entregar `Agendar pagamentos/recebimentos futuros com integração às previsões de tesouraria.` com resultado observável e verificável em review, alinhado com `RF63` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF5` e respeita as dependências `BK-MF5-02`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Agendar pagamentos/recebimentos futuros com integração às previsões de tesouraria.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF5` com rastreabilidade para `RF63`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Agendar pagamentos/recebimentos futuros com integração às previsões de tesouraria.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-08`.
