...
## Contexto do BK
- Entrega alvo: implementar `Suportar ≥ 25 utilizadores simultâneos por empresa sem degradação relevante.` com rastreabilidade direta ao requisito `RNF09`.
- Foco tecnico da macro: desempenho, seguranca e robustez tecnica.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Suportar ≥ 25 utilizadores simultâneos por empresa sem degradação relevante.` com autonomia técnica, garantindo cobertura do requisito `RNF09` e evidência objetiva para avaliação.
...
2. Validar dependencias técnicas (`-`) e preparar dados de teste mínimos para `Suportar ≥ 25 utilizadores simultâneos por empresa sem degradação relevante.`.
...
Contexto de rastreabilidade: `BK-MF6-02` -> `RNF09`.
