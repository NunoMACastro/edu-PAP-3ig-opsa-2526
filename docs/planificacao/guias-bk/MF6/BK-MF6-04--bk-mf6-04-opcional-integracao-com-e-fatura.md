# BK-MF6-04 - (Opcional) Integração com **e-Fatura**.
...
Neste BK vamos entregar `(Opcional) Integração com **e-Fatura**.` com resultado observável e verificável em review, alinhado com `RF44` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `(Opcional) Integração com **e-Fatura**.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RF44`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-05`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `(Opcional) Integração com **e-Fatura**.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-05`.
