# BK-MF6-07 - Registar **logs de integração** (uploads, SAF-T, reconciliações).
...
Neste BK vamos entregar `Registar **logs de integração** (uploads, SAF-T, reconciliações).` com resultado observável e verificável em review, alinhado com `RF58` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Registar **logs de integração** (uploads, SAF-T, reconciliações).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RF58`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Registar **logs de integração** (uploads, SAF-T, reconciliações).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-08`.
