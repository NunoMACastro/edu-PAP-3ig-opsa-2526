# BK-MF6-02 - Importar CSV/Excel de clientes, fornecedores, artigos e extratos.
...
Neste BK vamos entregar `Importar CSV/Excel de clientes, fornecedores, artigos e extratos.` com resultado observável e verificável em review, alinhado com `RF42` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Importar CSV/Excel de clientes, fornecedores, artigos e extratos.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RF42`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-03`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Importar CSV/Excel de clientes, fornecedores, artigos e extratos.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-03`.
