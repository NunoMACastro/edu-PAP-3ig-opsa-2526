# BK-MF6-13 - Geração de SAF-T conforme especificações legais (PT).

## Header
- `doc_id`: `GUIA-BK-MF6-13`
- `bk_id`: `BK-MF6-13`
- `macro`: `MF6`
- `owner`: `Oleksii`
- `apoio`: `Pedro`
...
Neste BK vamos entregar `Geração de SAF-T conforme especificações legais (PT).` com resultado observável e verificável em review, alinhado com `RNF25` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Geração de SAF-T conforme especificações legais (PT).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RNF25`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-14`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Geração de SAF-T conforme especificações legais (PT).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-14`.
