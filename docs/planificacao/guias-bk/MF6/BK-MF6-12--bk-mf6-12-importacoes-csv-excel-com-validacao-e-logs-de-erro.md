# BK-MF6-12 - Importações CSV/Excel com validação e logs de erro.
...
Neste BK vamos entregar `Importações CSV/Excel com validação e logs de erro.` com resultado observável e verificável em review, alinhado com `RNF24` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Importações CSV/Excel com validação e logs de erro.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RNF24`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-13`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Importações CSV/Excel com validação e logs de erro.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-13`.
