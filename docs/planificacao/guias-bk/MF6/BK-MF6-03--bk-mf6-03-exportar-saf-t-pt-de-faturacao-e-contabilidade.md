# BK-MF6-03 - Exportar **SAF-T (PT)** de faturação e contabilidade.
...
Neste BK vamos entregar `Exportar **SAF-T (PT)** de faturação e contabilidade.` com resultado observável e verificável em review, alinhado com `RF43` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Exportar **SAF-T (PT)** de faturação e contabilidade.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RF43`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-04`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Exportar **SAF-T (PT)** de faturação e contabilidade.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-04`.
