# BK-MF6-05 - Mapear documentos de integração para **centros de custo**.
...
Neste BK vamos entregar `Mapear documentos de integração para **centros de custo**.` com resultado observável e verificável em review, alinhado com `RF45` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 3` dentro de `MF6` e respeita as dependências `BK-MF2-10`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Mapear documentos de integração para **centros de custo**.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RF45`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-06`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Mapear documentos de integração para **centros de custo**.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-06`.
