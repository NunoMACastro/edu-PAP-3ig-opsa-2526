# BK-MF6-08 - Permitir **reabertura de períodos** apenas com registo de motivo.
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF1-08`
- `rf_rnf`: `RF59`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Permitir **reabertura de períodos** apenas com registo de motivo.` com resultado observável e verificável em review, alinhado com `RF59` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 3` dentro de `MF6` e respeita as dependências `BK-MF1-08`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Permitir **reabertura de períodos** apenas com registo de motivo.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RF59`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-09`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Permitir **reabertura de períodos** apenas com registo de motivo.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-09`.
