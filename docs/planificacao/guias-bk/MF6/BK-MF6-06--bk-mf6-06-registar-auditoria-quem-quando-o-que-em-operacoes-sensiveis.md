# BK-MF6-06 - Registar **auditoria**: quem, quando, o quê, em operações sensíveis.
...
Neste BK vamos entregar `Registar **auditoria**: quem, quando, o quê, em operações sensíveis.` com resultado observável e verificável em review, alinhado com `RF57` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Registar **auditoria**: quem, quando, o quê, em operações sensíveis.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF6` com rastreabilidade para `RF57`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-07`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Registar **auditoria**: quem, quando, o quê, em operações sensíveis.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-07`.
