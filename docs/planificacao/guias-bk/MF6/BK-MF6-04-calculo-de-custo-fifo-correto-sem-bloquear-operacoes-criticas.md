...
## Contexto do BK
- Entrega alvo: implementar `Cálculo de custo (FIFO) deve manter correção e não bloquear operações críticas.` com rastreabilidade direta ao requisito `RNF11`.
- Foco tecnico da macro: desempenho, seguranca e robustez tecnica.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Cálculo de custo (FIFO) deve manter correção e não bloquear operações críticas.` com autonomia técnica, garantindo cobertura do requisito `RNF11` e evidência objetiva para avaliação.
...
2. Validar dependencias técnicas (`-`) e preparar dados de teste mínimos para `Cálculo de custo (FIFO) deve manter correção e não bloquear operações críticas.`.
...
Contexto de rastreabilidade: `BK-MF6-04` -> `RNF11`.
