# BK-MF1-01 - Registo, login e logout com cookies HttpOnly.
...
Neste BK vamos entregar `Registo, login e logout com cookies HttpOnly.` com resultado observável e verificável em review, alinhado com `RF01` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Registo, login e logout com cookies HttpOnly.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF1` com rastreabilidade para `RF01`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-02`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Registo, login e logout com cookies HttpOnly.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-02`.
