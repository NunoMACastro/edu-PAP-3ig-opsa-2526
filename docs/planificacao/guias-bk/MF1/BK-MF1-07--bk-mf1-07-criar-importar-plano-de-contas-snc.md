# BK-MF1-07 - Criar/importar plano de contas (SNC).
...
Neste BK vamos entregar `Criar/importar plano de contas (SNC).` com resultado observável e verificável em review, alinhado com `RF07` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Criar/importar plano de contas (SNC).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF1` com rastreabilidade para `RF07`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar/importar plano de contas (SNC).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-08`.
