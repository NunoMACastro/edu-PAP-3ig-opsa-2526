...
```prisma
Reutiliza `JournalEntry`, `JournalEntryLine` e `JournalSource` criados no `BK-MF1-04`; garantir que `PURCHASE` existe no enum.
```
...
export async function postPurchaseDocument(prisma, companyId, userId, purchaseDocumentId) {
    return prisma.$transaction(async (tx) => {
        ...
        if (!["REGISTERED", "APPROVED", "POSTED", "PAID"].includes(document.status)) throw httpError(409, "INVALID_STATUS", "Compra sem estado contabilizavel");
