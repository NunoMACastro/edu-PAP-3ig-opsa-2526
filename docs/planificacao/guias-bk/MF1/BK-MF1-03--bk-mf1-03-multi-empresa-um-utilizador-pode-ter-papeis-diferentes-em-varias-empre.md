# BK-MF1-03 - Multi-empresa (um utilizador pode ter papéis diferentes em várias empresas).
...
Neste BK vamos entregar `Multi-empresa (um utilizador pode ter papéis diferentes em várias empresas).` com resultado observável e verificável em review, alinhado com `RF03` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF1` e respeita as dependências `BK-MF1-02`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Multi-empresa (um utilizador pode ter papéis diferentes em várias empresas).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF1` com rastreabilidade para `RF03`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-04`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Multi-empresa (um utilizador pode ter papéis diferentes em várias empresas).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-04`.
...
## Glossário rápido
- `auth`: conceito operativo central para este BK.
- `papel`: restrição técnica a garantir durante execução.
- `multi-empresa`: entidade funcional principal deste domínio.
- `sessão`: critério de controlo e validação desta entrega.
- `permissão`: evidência esperada para handoff e defesa.
