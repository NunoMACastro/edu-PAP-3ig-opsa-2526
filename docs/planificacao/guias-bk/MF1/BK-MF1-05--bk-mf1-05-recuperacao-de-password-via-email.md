# BK-MF1-05 - Recuperação de password via email.
...
Neste BK vamos entregar `Recuperação de password via email.` com resultado observável e verificável em review, alinhado com `RF05` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Recuperação de password via email.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF1` com rastreabilidade para `RF05`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-06`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Recuperação de password via email.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-06`.
