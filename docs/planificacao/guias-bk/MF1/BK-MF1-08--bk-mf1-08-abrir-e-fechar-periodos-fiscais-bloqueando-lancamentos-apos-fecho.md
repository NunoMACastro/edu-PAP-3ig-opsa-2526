# BK-MF1-08 - Abrir e fechar períodos fiscais, bloqueando lançamentos após fecho.
...
Neste BK vamos entregar `Abrir e fechar períodos fiscais, bloqueando lançamentos após fecho.` com resultado observável e verificável em review, alinhado com `RF08` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF1` e respeita as dependências `BK-MF1-07`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Abrir e fechar períodos fiscais, bloqueando lançamentos após fecho.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF1` com rastreabilidade para `RF08`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-01`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Abrir e fechar períodos fiscais, bloqueando lançamentos após fecho.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-01`.
