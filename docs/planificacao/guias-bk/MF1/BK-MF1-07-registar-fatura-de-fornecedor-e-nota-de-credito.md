...
## Contexto do BK
- Entrega alvo: implementar `Registar Fatura de Fornecedor e Nota de Crédito.` com rastreabilidade direta ao requisito `RF19`.
- Foco tecnico da macro: fluxo comercial (vendas/compras) com impacto contabilistico imediato.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Registar Fatura de Fornecedor e Nota de Crédito.` com autonomia técnica, garantindo cobertura do requisito `RF19` e evidência objetiva para avaliação.
...
Contexto de rastreabilidade: `BK-MF1-07` -> `RF19`.

```ts
type LinhaDocumento = { conta: string; base: number; taxaIVA: number };

export function validarDocumentoFiscal(linhas: LinhaDocumento[], bkId = 'BK-MF1-07') {
