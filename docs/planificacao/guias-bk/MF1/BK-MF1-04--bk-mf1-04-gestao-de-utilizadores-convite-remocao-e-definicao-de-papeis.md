# BK-MF1-04 - Gestão de utilizadores: convite, remoção e definição de papéis.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF1-03`
- `rf_rnf`: `RF04`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Gestão de utilizadores: convite, remoção e definição de papéis.` com resultado observável e verificável em review, alinhado com `RF04` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF1` e respeita as dependências `BK-MF1-03`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Gestão de utilizadores: convite, remoção e definição de papéis.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF1` com rastreabilidade para `RF04`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-05`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Gestão de utilizadores: convite, remoção e definição de papéis.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-05`.
