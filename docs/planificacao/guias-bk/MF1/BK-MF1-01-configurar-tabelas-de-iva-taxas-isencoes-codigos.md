...
## Bloco pedagogico
### Objetivo
Executar `Configurar tabelas de IVA (taxas, isenções, códigos).` com autonomia técnica, garantindo cobertura do requisito `RF13` e evidência objetiva para avaliação.
...
- [ ] Sei mostrar onde esta o requisito `RF13` no sistema.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF1-01` e o requisito `RF13`.
2. Validar dependencias técnicas (`-`) e preparar dados de teste mínimos para `Configurar tabelas de IVA (taxas, isenções, códigos).`.
...
export function validarDocumentoFiscal(linhas: LinhaDocumento[], bkId = 'BK-MF1-01') {
