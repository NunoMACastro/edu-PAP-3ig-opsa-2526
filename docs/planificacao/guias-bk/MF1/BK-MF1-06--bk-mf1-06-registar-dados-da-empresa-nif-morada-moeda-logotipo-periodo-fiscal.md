# BK-MF1-06 - Registar dados da empresa (NIF, morada, moeda, logótipo, período fiscal).
...
Neste BK vamos entregar `Registar dados da empresa (NIF, morada, moeda, logótipo, período fiscal).` com resultado observável e verificável em review, alinhado com `RF06` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 1` dentro de `MF1` e respeita as dependências `-`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Registar dados da empresa (NIF, morada, moeda, logótipo, período fiscal).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF1` com rastreabilidade para `RF06`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-07`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Registar dados da empresa (NIF, morada, moeda, logótipo, período fiscal).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-07`.
