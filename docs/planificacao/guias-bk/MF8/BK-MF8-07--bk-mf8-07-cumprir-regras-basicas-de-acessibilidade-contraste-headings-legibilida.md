# BK-MF8-07 - Cumprir regras básicas de acessibilidade (contraste, headings, legibilidade).
...
Neste BK vamos entregar `Cumprir regras básicas de acessibilidade (contraste, headings, legibilidade).` com resultado observável e verificável em review, alinhado com `RNF04` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Cumprir regras básicas de acessibilidade (contraste, headings, legibilidade).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RNF04`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Cumprir regras básicas de acessibilidade (contraste, headings, legibilidade).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-08`.
