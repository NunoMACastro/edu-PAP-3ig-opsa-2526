# BK-MF8-08 - Os formulários devem validar erros antes de submissão (NIF, IBAN, datas, IVA, contas SNC).
...
Neste BK vamos entregar `Os formulários devem validar erros antes de submissão (NIF, IBAN, datas, IVA, contas SNC).` com resultado observável e verificável em review, alinhado com `RNF05` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Os formulários devem validar erros antes de submissão (NIF, IBAN, datas, IVA, contas SNC).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RNF05`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-09`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Os formulários devem validar erros antes de submissão (NIF, IBAN, datas, IVA, contas SNC).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-09`.
