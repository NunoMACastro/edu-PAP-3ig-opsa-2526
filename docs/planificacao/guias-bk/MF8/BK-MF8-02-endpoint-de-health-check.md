...
## Contexto do BK
- Entrega alvo: implementar `Endpoint de health-check.` com rastreabilidade direta ao requisito `RNF31`.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do BK-MF8-02 e o requisito `RNF31`.
2. Verificar pre-condicoes tecnicas (-) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Endpoint de health-check.`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF8-02: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bk: 'BK-MF8-02', payload };
}
```
