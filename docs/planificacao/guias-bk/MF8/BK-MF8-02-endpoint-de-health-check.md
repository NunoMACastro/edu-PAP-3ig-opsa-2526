...
## Contexto do BK
- Entrega alvo: implementar `Endpoint de health-check.` com rastreabilidade direta ao requisito `RNF30`.
- Foco tecnico da macro: operacao final, i18n e fecho para defesa PAP.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Endpoint de health-check.` com autonomia técnica, garantindo cobertura do requisito `RNF30` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF8-02` e o requisito `RNF30`.
...
export function validarResultado(res: Resultado) {
  ...
  return { bk: 'BK-MF8-02', validado: true };
}
```
