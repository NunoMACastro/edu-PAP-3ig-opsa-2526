# BK-MF8-10 - Interface em português de Portugal.
...
Neste BK vamos entregar `Interface em português de Portugal.` com resultado observável e verificável em review, alinhado com `RNF38` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Interface em português de Portugal.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RNF38`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-11`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Interface em português de Portugal.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-11`.
