...
## Contexto do BK
- Entrega alvo: implementar `Deploy com rollback.` com rastreabilidade direta ao requisito `RNF32`.
- Foco tecnico da macro: operacao final, i18n e fecho para defesa PAP.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Conseguir explicar e executar o BK `BK-MF8-03` de forma autonoma, incluindo caminho feliz, negativos e evidencia pronta para defesa.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do BK-MF8-03 e o requisito `RNF32`.
2. Verificar pre-condicoes tecnicas (-) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Deploy com rollback.`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF8-03: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bk: 'BK-MF8-03', payload };
}
```
