# BK-MF8-05 - Layout responsivo (desktop > tablet > mobile) com grelhas e tabelas adaptadas.
...
Neste BK vamos entregar `Layout responsivo (desktop > tablet > mobile) com grelhas e tabelas adaptadas.` com resultado observável e verificável em review, alinhado com `RNF02` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Layout responsivo (desktop > tablet > mobile) com grelhas e tabelas adaptadas.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RNF02`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-06`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Layout responsivo (desktop > tablet > mobile) com grelhas e tabelas adaptadas.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-06`.
