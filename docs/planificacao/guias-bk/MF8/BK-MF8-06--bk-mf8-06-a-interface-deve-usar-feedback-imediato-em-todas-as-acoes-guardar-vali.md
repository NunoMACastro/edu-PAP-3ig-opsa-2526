# BK-MF8-06 - A interface deve usar feedback imediato em todas as ações (guardar, validar, uploads).
...
Neste BK vamos entregar `A interface deve usar feedback imediato em todas as ações (guardar, validar, uploads).` com resultado observável e verificável em review, alinhado com `RNF03` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `A interface deve usar feedback imediato em todas as ações (guardar, validar, uploads).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RNF03`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-07`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `A interface deve usar feedback imediato em todas as ações (guardar, validar, uploads).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-07`.
