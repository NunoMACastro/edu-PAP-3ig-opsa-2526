# BK-MF8-12 - Datas, moedas e separadores no padrão europeu.
...
Neste BK vamos entregar `Datas, moedas e separadores no padrão europeu.` com resultado observável e verificável em review, alinhado com `RNF40` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Datas, moedas e separadores no padrão europeu.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RNF40`.
- Impacto na sequência: desbloqueia a progressão para `fecho de cadeia`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Datas, moedas e separadores no padrão europeu.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `novas extensões fora da cadeia atual`.
...
- `-` (fim da cadeia atual de execução)
