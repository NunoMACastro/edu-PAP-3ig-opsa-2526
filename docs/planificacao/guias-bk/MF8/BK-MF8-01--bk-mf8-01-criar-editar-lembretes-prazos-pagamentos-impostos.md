# BK-MF8-01 - Criar/editar lembretes (prazos, pagamentos, impostos).
...
Neste BK vamos entregar `Criar/editar lembretes (prazos, pagamentos, impostos).` com resultado observável e verificável em review, alinhado com `RF54` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Criar/editar lembretes (prazos, pagamentos, impostos).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RF54`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-02`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar/editar lembretes (prazos, pagamentos, impostos).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-02`.
