# BK-MF8-04 - Interface intuitiva, clara e consistente entre módulos (Vendas, Compras, Inventário, Contabilidade).
...
Neste BK vamos entregar `Interface intuitiva, clara e consistente entre módulos (Vendas, Compras, Inventário, Contabilidade).` com resultado observável e verificável em review, alinhado com `RNF01` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Interface intuitiva, clara e consistente entre módulos (Vendas, Compras, Inventário, Contabilidade).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF8` com rastreabilidade para `RNF01`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-05`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Interface intuitiva, clara e consistente entre módulos (Vendas, Compras, Inventário, Contabilidade).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-05`.
