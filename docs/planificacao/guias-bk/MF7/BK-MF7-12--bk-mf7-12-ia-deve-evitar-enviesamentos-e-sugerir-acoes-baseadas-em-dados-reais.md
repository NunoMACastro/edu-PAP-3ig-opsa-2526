# BK-MF7-12 - IA deve evitar enviesamentos e sugerir ações baseadas em dados reais.
...
Neste BK vamos entregar `IA deve evitar enviesamentos e sugerir ações baseadas em dados reais.` com resultado observável e verificável em review, alinhado com `RNF37` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 3` dentro de `MF7` e respeita as dependências `-`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `IA deve evitar enviesamentos e sugerir ações baseadas em dados reais.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RNF37`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-01`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `IA deve evitar enviesamentos e sugerir ações baseadas em dados reais.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-01`.
