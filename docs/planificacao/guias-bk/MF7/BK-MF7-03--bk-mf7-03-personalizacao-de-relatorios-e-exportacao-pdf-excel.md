# BK-MF7-03 - Personalização de relatórios e exportação (PDF/Excel).
...
Neste BK vamos entregar `Personalização de relatórios e exportação (PDF/Excel).` com resultado observável e verificável em review, alinhado com `RF48` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Personalização de relatórios e exportação (PDF/Excel).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RF48`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-04`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Personalização de relatórios e exportação (PDF/Excel).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-04`.
