# BK-MF7-07 - Emitir **alertas inteligentes** (cashflow, desvios, ruturas).
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF5-03, BK-MF2-09`
- `rf_rnf`: `RF52`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Emitir **alertas inteligentes** (cashflow, desvios, ruturas).` com resultado observável e verificável em review, alinhado com `RF52` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 3` dentro de `MF7` e respeita as dependências `BK-MF5-03, BK-MF2-09`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Emitir **alertas inteligentes** (cashflow, desvios, ruturas).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RF52`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-08`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Emitir **alertas inteligentes** (cashflow, desvios, ruturas).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-08`.
