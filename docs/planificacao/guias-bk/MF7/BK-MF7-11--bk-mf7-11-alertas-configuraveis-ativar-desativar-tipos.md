# BK-MF7-11 - Alertas configuráveis (ativar/desativar tipos).
...
Neste BK vamos entregar `Alertas configuráveis (ativar/desativar tipos).` com resultado observável e verificável em review, alinhado com `RNF36` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Alertas configuráveis (ativar/desativar tipos).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RNF36`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-12`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Alertas configuráveis (ativar/desativar tipos).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-12`.
