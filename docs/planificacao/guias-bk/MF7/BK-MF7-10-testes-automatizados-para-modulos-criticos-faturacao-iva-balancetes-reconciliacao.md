...
## Contexto do BK
- Entrega alvo: implementar `Testes automatizados para módulos críticos (faturação, IVA, balancetes, reconciliação).` com rastreabilidade direta ao requisito `RNF27`.
- Foco tecnico da macro: compliance, interoperabilidade e modularidade.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Testes automatizados para módulos críticos (faturação, IVA, balancetes, reconciliação).` com autonomia técnica, garantindo cobertura do requisito `RNF27` e evidência objetiva para avaliação.
...
Contexto de rastreabilidade: `BK-MF7-10` -> `RNF27`.

```ts
type LinhaDocumento = { conta: string; base: number; taxaIVA: number };

export function validarDocumentoFiscal(linhas: LinhaDocumento[], bkId = 'BK-MF7-10') {
