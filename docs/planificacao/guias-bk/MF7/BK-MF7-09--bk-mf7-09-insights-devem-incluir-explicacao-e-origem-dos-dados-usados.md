# BK-MF7-09 - Insights devem incluir explicação e origem dos dados usados.
...
Neste BK vamos entregar `Insights devem incluir explicação e origem dos dados usados.` com resultado observável e verificável em review, alinhado com `RNF34` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `Insights devem incluir explicação e origem dos dados usados.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RNF34`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-10`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Insights devem incluir explicação e origem dos dados usados.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-10`.
