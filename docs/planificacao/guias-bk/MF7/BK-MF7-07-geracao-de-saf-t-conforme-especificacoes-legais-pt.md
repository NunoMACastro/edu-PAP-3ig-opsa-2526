...
## Bloco pedagogico
### Objetivo
Executar `Geração de SAF-T conforme especificações legais (PT).` com autonomia técnica, garantindo cobertura do requisito `RNF24` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF7-07` e o requisito `RNF24`.
2. Validar dependencias técnicas (`-`) e preparar dados de teste mínimos para `Geração de SAF-T conforme especificações legais (PT).`.
