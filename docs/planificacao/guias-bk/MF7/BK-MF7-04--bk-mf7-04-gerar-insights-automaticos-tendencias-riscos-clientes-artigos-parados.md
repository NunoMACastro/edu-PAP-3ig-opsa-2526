# BK-MF7-04 - Gerar **insights automáticos** (tendências, riscos, clientes, artigos parados).
...
Neste BK vamos entregar `Gerar **insights automáticos** (tendências, riscos, clientes, artigos parados).` com resultado observável e verificável em review, alinhado com `RF49` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 3` dentro de `MF7` e respeita as dependências `BK-MF7-01`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Gerar **insights automáticos** (tendências, riscos, clientes, artigos parados).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RF49`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-05`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Gerar **insights automáticos** (tendências, riscos, clientes, artigos parados).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-05`.
...
## Glossário rápido
- `dashboard`: conceito operativo central para este BK.
- `KPI`: restrição técnica a garantir durante execução.
- `insight`: entidade funcional principal deste domínio.
- `explicabilidade`: critério de controlo e validação desta entrega.
- `alerta`: evidência esperada para handoff e defesa.
