# BK-MF7-05 - Sugerir **ações** (ajustar preços, negociar fornecedor, repor stock).
...
Neste BK vamos entregar `Sugerir **ações** (ajustar preços, negociar fornecedor, repor stock).` com resultado observável e verificável em review, alinhado com `RF50` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 3` dentro de `MF7` e respeita as dependências `BK-MF7-04`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Sugerir **ações** (ajustar preços, negociar fornecedor, repor stock).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RF50`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-06`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Sugerir **ações** (ajustar preços, negociar fornecedor, repor stock).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-06`.
