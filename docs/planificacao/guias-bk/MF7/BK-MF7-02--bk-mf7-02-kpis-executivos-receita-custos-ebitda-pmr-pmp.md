# BK-MF7-02 - KPIs executivos (receita, custos, EBITDA, PMR, PMP).
...
Neste BK vamos entregar `KPIs executivos (receita, custos, EBITDA, PMR, PMP).` com resultado observável e verificável em review, alinhado com `RF47` e com os critérios de aceite definidos neste guia.
...
- Impacto funcional: este BK materializa `KPIs executivos (receita, custos, EBITDA, PMR, PMP).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF7` com rastreabilidade para `RF47`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-03`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `KPIs executivos (receita, custos, EBITDA, PMR, PMP).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-03`.
