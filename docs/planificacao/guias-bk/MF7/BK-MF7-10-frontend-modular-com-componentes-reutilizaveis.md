...
## Contexto do BK
- Entrega alvo: implementar `Frontend modular com componentes reutilizáveis.` com rastreabilidade direta ao requisito `RNF28`.
- Foco tecnico da macro: compliance, interoperabilidade e modularidade.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Frontend modular com componentes reutilizáveis.` com rastreabilidade explicita para `RNF28` e demonstracao tecnica no contexto da sprint `S11-S12`.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do BK-MF7-10 e o requisito `RNF28`.
2. Verificar pre-condicoes tecnicas (-) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Frontend modular com componentes reutilizáveis.`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF7-10: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bk: 'BK-MF7-10', payload };
}
```
