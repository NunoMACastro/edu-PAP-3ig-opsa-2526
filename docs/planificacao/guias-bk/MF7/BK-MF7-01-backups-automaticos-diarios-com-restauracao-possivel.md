...
## Contexto do BK
- Entrega alvo: implementar `Backups automáticos diários com restauração possível.` com rastreabilidade direta ao requisito `RNF19`.
- Foco tecnico da macro: compliance, interoperabilidade e modularidade.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Backups automáticos diários com restauração possível.` com rastreabilidade explicita para `RNF19` e demonstracao tecnica no contexto da sprint `S11-S12`.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do BK-MF7-01 e o requisito `RNF19`.
2. Verificar pre-condicoes tecnicas (-) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Backups automáticos diários com restauração possível.`.
