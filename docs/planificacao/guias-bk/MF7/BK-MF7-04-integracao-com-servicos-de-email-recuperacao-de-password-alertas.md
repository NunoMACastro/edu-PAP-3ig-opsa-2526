...
## Bloco pedagogico
### Objetivo
Executar `Integração com serviços de email (recuperação de password, alertas).` com autonomia técnica, garantindo cobertura do requisito `RNF21` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF7-04` e o requisito `RNF21`.
2. Validar dependencias técnicas (`-`) e preparar dados de teste mínimos para `Integração com serviços de email (recuperação de password, alertas).`.
...
// Exemplo de uso no endpoint associado ao requisito RNF21
