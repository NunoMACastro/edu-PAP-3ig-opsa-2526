# BK-MF4-04 - Gerar **Mapas de IVA** (liquidado/dedutível).
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF3-03, BK-MF3-10`
- `rf_rnf`: `RF37`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Gerar **Mapas de IVA** (liquidado/dedutível).` com resultado observável e verificável em review, alinhado com `RF37` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF4` e respeita as dependências `BK-MF3-03, BK-MF3-10`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Gerar **Mapas de IVA** (liquidado/dedutível).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF4` com rastreabilidade para `RF37`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-01`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Gerar **Mapas de IVA** (liquidado/dedutível).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-01`.
...
## Glossário rápido
- `balancete`: conceito operativo central para este BK.
- `razão`: restrição técnica a garantir durante execução.
- `demonstração financeira`: entidade funcional principal deste domínio.
- `lançamento`: critério de controlo e validação desta entrega.
- `compliance`: evidência esperada para handoff e defesa.
