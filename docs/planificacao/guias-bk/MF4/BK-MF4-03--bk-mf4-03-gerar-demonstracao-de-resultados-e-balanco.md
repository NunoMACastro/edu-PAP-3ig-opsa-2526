# BK-MF4-03 - Gerar **Demonstração de Resultados e Balanço**.
...
- `estado`: `TODO`
- `esforço`: `L`
- `dependências`: `BK-MF4-02`
- `rf_rnf`: `RF36`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar `Gerar **Demonstração de Resultados e Balanço**.` com resultado observável e verificável em review, alinhado com `RF36` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF4` e respeita as dependências `BK-MF4-02`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Gerar **Demonstração de Resultados e Balanço**.` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF4` com rastreabilidade para `RF36`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF4-04`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Gerar **Demonstração de Resultados e Balanço**.` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF4-04`.
