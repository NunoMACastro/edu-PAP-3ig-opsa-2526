...
## Contexto do BK
- Entrega alvo: implementar `Sugerir ações (ajustar preços, negociar fornecedor, repor stock).` com rastreabilidade direta ao requisito `RF40`.
- Foco tecnico da macro: inteligencia operacional, alertas e governanca de operacoes.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Sugerir ações (ajustar preços, negociar fornecedor, repor stock).` com autonomia técnica, garantindo cobertura do requisito `RF40` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF4-02` e o requisito `RF40`.
...
Contexto de rastreabilidade: `BK-MF4-02` -> `RF40`.
