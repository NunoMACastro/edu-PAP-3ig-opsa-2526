...
## Contexto do BK
- Entrega alvo: implementar `Emitir alertas inteligentes (cashflow, desvios, ruturas).` com rastreabilidade direta ao requisito `RF52`.
- Foco tecnico da macro: inteligencia operacional, alertas e governanca de operacoes.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Emitir alertas inteligentes (cashflow, desvios, ruturas).` com rastreabilidade explicita para `RF52` e demonstracao tecnica no contexto da sprint `S08-S09`.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do BK-MF4-04 e o requisito `RF52`.
2. Verificar pre-condicoes tecnicas (BK-MF3-04, BK-MF2-06) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Emitir alertas inteligentes (cashflow, desvios, ruturas).`.
