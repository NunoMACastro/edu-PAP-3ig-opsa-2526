...
## Contexto do BK
- Entrega alvo: implementar `Permitir reabertura de períodos apenas com registo de motivo.` com rastreabilidade direta ao requisito `RF58`.
- Foco tecnico da macro: inteligencia operacional, alertas e governanca de operacoes.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Permitir reabertura de períodos apenas com registo de motivo.` com autonomia técnica, garantindo cobertura do requisito `RF58` e evidência objetiva para avaliação.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do `BK-MF4-11` e o requisito `RF58`.
2. Validar dependencias técnicas (`BK-MF0-08`) e preparar dados de teste mínimos para `Permitir reabertura de períodos apenas com registo de motivo.`.
...
export function validarResultado(res: Resultado) {
  ...
  return { bk: 'BK-MF4-11', validado: true };
}
```
