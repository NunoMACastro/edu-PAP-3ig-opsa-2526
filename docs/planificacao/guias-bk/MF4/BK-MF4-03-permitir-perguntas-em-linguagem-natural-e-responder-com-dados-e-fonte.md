...
## Contexto do BK
- Entrega alvo: implementar `Permitir perguntas em linguagem natural e responder com dados e fonte.` com rastreabilidade direta ao requisito `RF51`.
- Foco tecnico da macro: inteligencia operacional, alertas e governanca de operacoes.
- Regra de governanca: nao alterar IDs nem contratos de dados (`bk_id/mf/sprint/owner/rf_rnf/deps/guia_path/core_or_reforco`).

## Bloco pedagogico
### Objetivo
Executar `Permitir perguntas em linguagem natural e responder com dados e fonte.` com rastreabilidade explicita para `RF51` e demonstracao tecnica no contexto da sprint `S08-S09`.

### Pre-requisitos
- Ler o requisito `RF51` e rever o contexto em `MATRIZ-CANONICA-BK.md` e `BACKLOG-MVP.md`.
...
- [ ] Sei mostrar onde esta o requisito `RF51` no sistema.
...
### Passos
1. Confirmar no `BACKLOG-MVP` e na `MATRIZ-CANONICA-BK` o escopo do BK-MF4-03 e o requisito `RF51`.
2. Verificar pre-condicoes tecnicas (BK-MF3-10) e validar ambiente local antes de implementar.
3. Definir contrato de entrada/saida do fluxo principal para `Permitir perguntas em linguagem natural e responder com dados e fonte.`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF4-03: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bk: 'BK-MF4-03', payload };
}
```
