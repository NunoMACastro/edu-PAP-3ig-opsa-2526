# BK-MF4-01 - Criar e editar **lançamentos manuais** (com anexos).
...
Neste BK vamos entregar `Criar e editar **lançamentos manuais** (com anexos).` com resultado observável e verificável em review, alinhado com `RF34` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF4` e respeita as dependências `BK-MF1-07`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Criar e editar **lançamentos manuais** (com anexos).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF4` com rastreabilidade para `RF34`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF4-02`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Criar e editar **lançamentos manuais** (com anexos).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF4-02`.
