# BK-MF4-02 - Consultar **balancete e razão** exportável (PDF/Excel).
...
Neste BK vamos entregar `Consultar **balancete e razão** exportável (PDF/Excel).` com resultado observável e verificável em review, alinhado com `RF35` e com os critérios de aceite definidos neste guia.

A execução decorre em `Fase 2` dentro de `MF4` e respeita as dependências `BK-MF4-01`. Qualquer detalhe técnico não especificado nesta fase fica `a definir no BK dependente`.

## Porque isto é importante
- Impacto funcional: este BK materializa `Consultar **balancete e razão** exportável (PDF/Excel).` no produto OPSA.
- Impacto técnico/arquitetural: consolida capacidades da macro `MF4` com rastreabilidade para `RF35`.
- Impacto na sequência: desbloqueia a progressão para `BK-MF4-03`.
- Risco de execução incorreta: falhas aqui propagam inconsistências para backlog, validação e defesa.

## O que entra (scope)
- Definir tarefas executáveis para `Consultar **balancete e razão** exportável (PDF/Excel).` com owner e apoio claros.
...
- Trabalho de BKs seguintes, incluindo `BK-MF4-03`.
