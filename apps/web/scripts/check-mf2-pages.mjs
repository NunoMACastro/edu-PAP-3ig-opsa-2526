/**
 * @file Smoke checks textuais para garantir que os ecrãs MF2 continuam a expor os fluxos esperados.
 */
...
/**
 * Documenta a função assertIncludes no contexto deste módulo.
 *
 * @param source - Conteúdo onde o texto esperado é procurado.
 * @param expected - Texto esperado no ficheiro ou conteúdo analisado.
 * @param context - Contexto operacional necessário para a validação.
 * @returns Não devolve valor; lança erro se o texto obrigatório não existir.
 */
...
/**
 * Documenta a função assertContract no contexto deste módulo.
 *
 * @param context - Contexto operacional necessário para a validação.
 * @param source - Conteúdo onde o texto esperado é procurado.
 * @param expectedValues - Conjunto de valores obrigatórios a verificar.
 * @returns Não devolve valor; lança erro se faltar algum texto obrigatório.
 */
...
const contracts = [
  ...
  },
  {
    ...
    checks: [
      {
        source: app,
        values: ["InventoryCountPage", "Contagens físicas"],
      },
      {
        source: mf2Pages,
        values: [
  ...
  },
  {
    ...
    checks: [
      {
        source: app,
        values: ["ManualJournalPage", "Lançamentos manuais"],
      },
      {
        source: mf2Pages,
        values: [
          ...
          "apiClient.manualJournals.downloadAttachment",
          "Anexos disponíveis",
        ],
      },
      {
  ...
  },
  {
    ...
    checks: [
      {
        source: app,
        values: ["AccountingReportsPage", "Balancete e razão"],
      },
      {
        source: mf2Pages,
        values: [
      ...
      },
      {
        ...
        values: [
          ...
          "/accounting/reports/ledger/export",
  ...
  },
  {
    ...
    checks: [
      {
        source: app,
        values: ["FinancialStatementsPage", "DR e Balanço"],
      },
      {
        source: mf2Pages,
        values: [
