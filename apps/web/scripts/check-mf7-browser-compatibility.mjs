...
/**
 * Lê um ficheiro do frontend a partir da raiz de `real_dev/web`.
 *
 * @param {string} relativePath - Caminho relativo dentro de `real_dev/web`.
...
function assertNoBrowserBranch(relativePath, source) {
  for (const { pattern, reason } of forbiddenPatterns) {
    // O gate falha cedo para impedir caminhos divergentes por browser.
    assert.equal(
      pattern.test(source),
      false,
...
function assertRequiredContracts() {
  for (const { file, pattern, reason } of requiredContracts) {
    ...
    // Estes contratos protegem sessão, foco por teclado e layout responsivo.
