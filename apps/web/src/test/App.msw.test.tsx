...
describe("App com transporte MSW", () => {
  ...
  it("auditoria e integração tornam a segunda página acessível", async () => {
      ...
      http.get("http://localhost/api/audit/logs", ({ request }) => {
        ...
        return HttpResponse.json({
          items: [{
            ...
            actor: { id: "user-logs", name: "Utilizador de logs", email: "logs@example.test" },
            reference: { type: "OTHER", id: "company-msw", label: "Empresa" },
          }],
          pageInfo: cursor
    ...
    expect(await screen.findByText("First audit event")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Carregar mais" }));
    expect(await screen.findByText("Second audit event")).toBeInTheDocument();
    auditView.unmount();
  ...
  it("AUDITOR consulta histórico, lembretes e notificações sem criar compras", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Documentos de compra" }))
      .toBeInTheDocument();
    expect(await screen.findAllByRole("button", { name: "Histórico" })).toHaveLength(2);
