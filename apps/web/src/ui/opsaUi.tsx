...
export function AiSourceQualityPanel({ quality }: AiSourceQualityPanelProps) {
  ...
  return (
    <aside className="aiSourceQuality" aria-label="Qualidade da fonte da IA">
      <div className="aiSourceQuality__header">
        <strong>Qualidade da fonte</strong>
        {/* A confiança é apresentada como texto para não depender apenas da cor. */}
        <ModuleBadge label={`Confiança ${confidenceLabel}`} tone="warning" />
      </div>
      <p><strong>Fonte:</strong> {quality.source.label}</p>
      <p>{quality.limitation}</p>
      <details className="aiSourceQuality__technical">
        <summary>Ver referência técnica</summary>
        <code>{quality.source.type}/{quality.source.id}</code>
      </details>
    </aside>
  );
}
