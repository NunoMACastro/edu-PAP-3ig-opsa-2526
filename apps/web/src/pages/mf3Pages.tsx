...
function saftPeriodStatus(status?: string) {
  return status === "OPEN" ? "Aberto" : status === "CLOSED" ? "Fechado" : "Estado desconhecido";
}

function saftExportStatus(status: SaftExportSummary["status"]) {
  const labels: Record<SaftExportSummary["status"], string> = {
    PENDING: "Em fila",
    GENERATING: "A gerar o XML",
    VALIDATING: "A validar",
    READY: "Pronta para descarregar",
    FAILED: "Falhou",
  };
...
export function SaftExportPage() {
  ...
  useEffect(() => {
    ...
    const eligible = periods.filter(
      (period) => runtimeStatus.mode !== "EXTERNAL" || period.status === "CLOSED",
    );
    setPeriodId((current) =>
      eligible.some((period) => period.id === current)
        ? current
        : eligible[0]?.id ?? "",
    );
  }, [periods, runtimeStatus]);
    ...
    try {
      await action.run(
        async () => {
          ...
          if (runtimeStatus?.mode === "EXTERNAL" && selected?.status !== "CLOSED") {
            throw new Error("No modo externo só podes exportar um período fiscal fechado.");
          }
  ...
  const selectedEndDate = selectedPeriod?.endDate.slice(0, 10) ?? null;
  const estimatedCutoff = selectedPeriod?.status === "OPEN" && runtimeStatus?.mode === "ACADEMIC"
    ? [selectedEndDate, saftLocalDate()].filter(Boolean).sort()[0] ?? null
    : selectedEndDate;
  const modeLabel = runtimeStatus?.mode === "ACADEMIC"
    ? "Académico"
  ...
  return (
    ...
    >
      {runtimeStatus ? (
        <StatusMessage tone="neutral" title={`Modo atual: ${modeLabel}`}>
          {runtimeStatus.mode === "EXTERNAL"
            ? "Exige período fechado, validação XSD e revisão externa antes do download."
            : runtimeStatus.mode === "ACADEMIC"
              ? "Permite demonstrar um snapshot interno sem lhe atribuir validade legal."
              : "A exportação está desativada neste ambiente."}
        </StatusMessage>
      ) : null}
      {runtimeStatus?.mode === "ACADEMIC" ? (
        <StatusMessage tone="warning" title="Demonstração académica — não certificado">
          O XML permite demonstrar o fluxo e a reconciliação interna, mas não foi validado por XSD,
          não foi revisto externamente e não tem validade legal.
        </StatusMessage>
      ) : null}
            ...
            >
              ...
              {periods.map((period) => (
                <option
                  value={period.id}
                  key={period.id}
                  disabled={runtimeStatus?.mode === "EXTERNAL" && period.status !== "CLOSED"}
                >
                  {period.name} — {saftPeriodStatus(period.status)} ({period.startDate.slice(0, 10)} — {period.endDate.slice(0, 10)})
                </option>
              ))}
            </select>
          </label>
      ...
      <section className="operation" aria-labelledby="saft-preflight-title">
        <h3 id="saft-preflight-title">Pré-validação do snapshot</h3>
        {selectedPeriod ? (
          <dl className="structuredResult__details">
            <div>
              <dt>Período</dt>
              <dd>{selectedPeriod.name} — {saftPeriodStatus(selectedPeriod.status)}</dd>
            </div>
            <div>
              <dt>Data de corte prevista</dt>
              <dd>{estimatedCutoff ?? "Por determinar"}</dd>
            </div>
            <div>
              <dt>Verificações antes da criação</dt>
              <dd>Perfil fiscal, documentos, diário, contas, IVA e reconciliação interna</dd>
            </div>
          </dl>
        ) : (
          <p>
            {runtimeStatus?.mode === "EXTERNAL"
              ? "Não existe um período fechado elegível para o modo externo."
              : "Seleciona um período fiscal para preparar a exportação."}
          </p>
        )}
      </section>
      {action.feedback.message ? (
      ...
      {exportJob ? (
        <section className="operation" aria-labelledby="saft-export-state">
          ...
          <p>{saftExportStatus(exportJob.status)}</p>
          <dl className="structuredResult__details">
            <div>
              <dt>Intervalo exportado</dt>
              <dd>{exportJob.fromDate ?? "—"} — {exportJob.toDate ?? "—"}</dd>
            </div>
            <div>
              <dt>Data de corte</dt>
              <dd>{exportJob.asOfDate ?? exportJob.toDate ?? "—"}</dd>
            </div>
            <div>
              <dt>Reconciliação interna</dt>
              <dd>{exportJob.validation?.totalsStatus === "VALID" ? "Concluída" : "Pendente"}</dd>
            </div>
            <div>
              <dt>Validação externa</dt>
              <dd>
                {exportJob.validationMode === "ACADEMIC"
                  ? "Não executada neste modo"
                  : exportJob.validation?.xsdStatus === "VALID"
                    ? "Concluída"
                    : "Pendente"}
              </dd>
            </div>
          ...
          {exportJob.validationMode === "ACADEMIC" ? (
            <StatusMessage tone="warning" title="Snapshot académico sem validade legal">
              {exportJob.warning ?? "Uso exclusivo para demonstração académica; não certificado."}
            </StatusMessage>
          ) : null}
