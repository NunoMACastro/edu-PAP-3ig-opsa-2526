...
import { formatPortugueseDateTime, formatQuantity } from "../lib/formatters";
...
const STOCK_BALANCE_COLUMNS: ResourceColumn[] = [
  { key: "itemLabel", label: "Artigo", priority: "primary" },
  { key: "warehouseLabel", label: "Armazém", priority: "secondary" },
  {
    key: "quantity",
    label: "Quantidade",
    priority: "secondary",
    format: (value) => formatQuantity(typeof value === "string" || typeof value === "number" ? value : null),
  },
  {
    key: "updatedAt",
    label: "Atualizado em",
    priority: "desktop",
    format: (value) => typeof value === "string" ? formatPortugueseDateTime(value) : "-",
  },
];

/**
 * Mostra o saldo materializado por artigo e armazém sem calcular valores
 * monetários ou thresholds que o endpoint não fornece.
 */
export function StockBalancesPage() {
  ...
  const [filters, setFilters] = useState<{ itemId?: string; warehouseId?: string }>({});
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function load(
    append = false,
    nextFilters: { itemId?: string; warehouseId?: string } = filters,
  ) {
    ...
    setError(null);
    try {
      const page = await apiClient.inventory.listStockBalances({
        ...nextFilters,
        cursor,
        limit: 50,
      });
      ...
      if (!append) setFilters(nextFilters);
    } catch (caught) {
      setError(formatError(caught));
      if (!append) {
        setItems([]);
  ...
  const rows: TableRow[] = items.map((balance) => ({
    id: `${balance.item.id}:${balance.warehouse.id}`,
    itemLabel: `${balance.item.sku} — ${balance.item.name}`,
    warehouseLabel: `${balance.warehouse.code} — ${balance.warehouse.name}`,
  ...
  return (
    <PageFrame
      title="Stock atual"
      description="Saldo persistido por artigo e armazém. Atualiza esta página depois de contabilizar uma venda ou compra."
      actions={<button type="button" disabled={busy} onClick={() => void load(false, filters)}>Atualizar</button>}
    >
      <Feedback busy={busy} error={error ?? references.error} />
      <form
        className="search stockBalanceFilters"
        onSubmit={(event) => {
      ...
      >
        <ReferenceSelect name="itemId" label="Artigo" options={references.items} />
        <ReferenceSelect name="warehouseId" label="Armazém" options={references.warehouses} />
      ...
      {!busy && !error ? (
        <ResponsiveDataTable
          ...
          caption="Saldos de stock atuais"
          renderMobileTitle={(row) => String(row.itemLabel)}
          emptyState={(
            <StatusMessage tone="neutral" title="Sem saldo para apresentar">
              Ainda não existem movimentos para estes filtros. Prepara o produto demo ou regista uma entrada real.
            </StatusMessage>
          )}
        />
      ) : null}
      <CursorPaginationButton
        ...
        busy={busy}
        label="saldos de stock"
        onLoadMore={() => load(true)}
      />
      <p><Link to="/inventory/movements">Consultar movimentos que explicam estes saldos</Link></p>
    </PageFrame>
  );
}
