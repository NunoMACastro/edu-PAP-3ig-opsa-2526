...
function DateRangeForm({
  ...
  successMessage,
  errorMessage,
  defaultFrom = firstLocalDayOfMonth(),
    ...
    <form className="operation" onSubmit={submit}>
      ...
      <div className="fields">
        <label>
          ...
          <input name="from" type="date" required defaultValue={defaultFrom} />
        </label>
        <label>
      ...
      <button type="submit" disabled={action.busy}>
        {action.busy ? "A consultar..." : submitLabel}
      </button>
    </form>
  );
}
...
function SimpleList<TItem>({
  ...
  return <div className={`operationGrid${className ? ` ${className}` : ""}`}>{items.map(render)}</div>;
}

const AI_SUGGESTION_ACTION_LABELS: Record<string, string> = {
  REVIEW_PRICING: "Rever preços e margens",
  NEGOTIATE_CUSTOMER: "Rever cobrança com o cliente",
  REVIEW_STOCK: "Rever níveis de stock",
  REVIEW_CASHFLOW: "Rever tesouraria e cashflow",
};

const AI_SUGGESTION_STATUS_PRESENTATION: Record<
  string,
  { label: string; tone: "neutral" | "success" | "warning" }
> = {
  OPEN: { label: "Pendente", tone: "warning" },
...
/**
 * Traduz o tipo interno permitido pelo guardrail para uma ação curta em PT-PT.
 *
 * @param actionType - Código de recomendação devolvido pela API.
 * @returns Rótulo legível para o utilizador.
 */
function formatAiSuggestionAction(actionType: string) {
  return AI_SUGGESTION_ACTION_LABELS[actionType] ?? formatStatusLabel(actionType);
}

/**
 * Resolve o estado visual da sugestão sem alterar o lifecycle definido no backend.
 *
 * @param status - Estado persistido da sugestão.
 * @returns Rótulo e tom usados no distintivo.
 */
function aiSuggestionStatusPresentation(status: string) {
  return AI_SUGGESTION_STATUS_PRESENTATION[status] ?? {
...
export function AiSuggestionsPage() {
  ...
  return (
    ...
    >
      ...
      <ActionFeedbackMessage feedback={action.feedback} />
      <SimpleList
        className="aiSuggestionsGrid"
        items={suggestions}
        render={(item) => {
          const status = aiSuggestionStatusPresentation(item.status);
          return (
            <article className="operation aiSuggestionCard" key={item.id}>
              <header className="aiSuggestionCard__header">
                <h3>{item.title}</h3>
                <ModuleBadge label={status.label} tone={status.tone} />
              </header>
              <p className="aiSuggestionCard__action">
                <strong>Ação sugerida:</strong> {formatAiSuggestionAction(item.actionType)}
              </p>
              <details className="aiSuggestionCard__technical">
                <summary>Ver fundamento técnico</summary>
                <p>{item.rationale}</p>
              </details>
              ...
              {canManage && item.status === "OPEN" ? (
                <div className="actionToolbar aiSuggestionCard__actions">
                  <button type="button" onClick={async () => { await updateAiSuggestionStatus(item.id, "ACCEPTED", "USEFUL"); setSuggestions((current) => current.map((entry) => entry.id === item.id ? { ...entry, status: "ACCEPTED" } : entry)); }}>Aceitar</button>
                  <button type="button" onClick={async () => { await updateAiSuggestionStatus(item.id, "DISMISSED", "NOT_USEFUL"); setSuggestions((current) => current.map((entry) => entry.id === item.id ? { ...entry, status: "DISMISSED" } : entry)); }}>Dispensar</button>
                </div>
              ) : null}
            </article>
          );
 ...
 */
function rollingWindowStart(windowDays = 90) {
  const date = new Date();
  date.setDate(date.getDate() - (windowDays - 1));
  return toLocalDateInputValue(date);
}

function alertRecommendation() {
  return "Confirmar os factos com um responsável antes de tomar uma decisão.";
}
...
export function SmartAlertsPage() {
    ...
    const poll = async () => {
      try {
        ...
        if (run.status === "COMPLETED") {
          const result = await getSmartAlerts(runRange.from, runRange.to);
          if (!cancelled) {
        ...
        // Uma falha transitória de polling é repetida dentro do limite abaixo.
      }

      attempts += 1;
      if (attempts >= 30) {
        if (!cancelled) setRunStatus("TIMEOUT");
        return;
      }
  ...
  return (
    <PageFrame
      ...
      description="Análise determinística dos dados da empresa, baseada em regras e histórico; não executa ações."
    >
      {canRunAnalysis ? <DateRangeForm
        label="Período da análise"
        defaultFrom={rollingWindowStart()}
        submitLabel="Analisar dados da empresa"
        startMessage="A agendar análise..."
        successMessage="Análise agendada para o worker."
        errorMessage="Não foi possível agendar a análise."
        onSubmit={async (from, to) => {
          const run = await createAiAnalysisRun(from, to);
          setAlerts([]);
          setRunId(run.run.id);
      ...
      {runStatus ? (
        <StatusMessage
          tone={["FAILED", "TIMEOUT"].includes(runStatus) ? "danger" : "neutral"}
          title="Estado da análise"
        >
          {runStatus === "TIMEOUT"
            ? "O tempo de espera terminou. Atualiza a página para confirmar o resultado."
            : formatStatusLabel(runStatus)}
          {["QUEUED", "RUNNING"].includes(runStatus)
            ? " · A análise está a ser processada."
            : ""}
        </StatusMessage>
      ) : (
        <StatusMessage tone="neutral" title="Ainda sem análise">
          Executa a janela sugerida de 90 dias para procurar riscos com evidência suficiente.
        </StatusMessage>
      )}
      {runStatus === "COMPLETED" && alerts.length === 0 ? (
        <StatusMessage tone="success" title="Sem riscos detetados">
          Nenhuma regra reuniu simultaneamente condições de risco e amostra suficiente. Uma amostra curta ou sem saídas não gera uma previsão inventada.
        </StatusMessage>
      ) : null}
      {alerts.length > 0 ? (
          ...
          {alerts.map((item) => {
            ...
            return (
              <article className="operation" key={item.id}>
                <header>
                  <h3>{item.title}</h3>
                  <p>{formatStatusLabel(item.severity)} · {formatStatusLabel(item.status)}</p>
                </header>
                <p>{item.message}</p>
                <dl>
                  <div>
                    <dt>Período</dt>
                    <dd>{item.periodFrom && item.periodTo ? `${formatPortugueseDate(item.periodFrom)} a ${formatPortugueseDate(item.periodTo)}` : "Período não indicado"}</dd>
                  </div>
                  {metrics?.balance !== undefined ? <div><dt>Saldo</dt><dd>{formatQuantity(metrics.balance)}</dd></div> : null}
                  {metrics?.averageDailyOutflow !== undefined ? <div><dt>Saída média diária</dt><dd>{formatQuantity(metrics.averageDailyOutflow)}</dd></div> : null}
                  {metrics?.daysOfCover !== undefined ? <div><dt>Cobertura estimada</dt><dd>{metrics.daysOfCover === null ? "Sem estimativa" : `${formatQuantity(metrics.daysOfCover)} dias`}</dd></div> : null}
                  {metrics?.observedDays !== undefined ? <div><dt>Amostra</dt><dd>{metrics.observedDays} dias · {metrics.outflowCount ?? 0} saídas</dd></div> : null}
                </dl>
                <p><strong>Explicação:</strong> {item.evidence?.formula ?? "Resultado calculado pelas regras determinísticas da OPSA."}</p>
                <p><strong>Recomendação:</strong> {alertRecommendation()}</p>
                {item.evidence?.limitations?.length ? (
                  <details>
                    <summary>Limitações da análise</summary>
                    <ul>{item.evidence.limitations.map((limitation) => <li key={limitation}>{limitation}</li>)}</ul>
                  </details>
                ) : null}
                <p>
                  Fonte: {item.sourceLabel}
                  {item.evidence?.quality ? ` · ${formatStatusLabel(item.evidence.quality)}` : ""}
                  {item.ruleCode ? ` · Regra ${item.ruleCode} v${item.ruleVersion ?? 1}` : ""}
                </p>
                <StatusMessage tone="neutral" title="Decisão humana obrigatória">
                  A análise recomenda uma revisão; não cria documentos nem altera stock ou contabilidade.
                </StatusMessage>
                {canManage ? <div className="inlineActions"><button type="button" onClick={async () => { await updateSmartAlertStatus(item.id, "ACKNOWLEDGED"); setAlerts((current) => current.map((entry) => entry.id === item.id ? { ...entry, status: "ACKNOWLEDGED" } : entry)); }}>Reconhecer</button>
                <button type="button" onClick={async () => { await updateSmartAlertStatus(item.id, "RESOLVED"); setAlerts((current) => current.map((entry) => entry.id === item.id ? { ...entry, status: "RESOLVED" } : entry)); }}>Resolver</button></div> : null}
              </article>
            );
          })}
        </div>
...
function auditDetailEntries(details: AuditLogItem["details"]) {
  if (!details) return [];
  return Object.entries(details).filter(([key, value]) => {
    const normalizedKey = key.toLocaleLowerCase();
    const safeValue = value === null || ["string", "number", "boolean"].includes(typeof value);
    return safeValue && !/(id|hash|token|secret|password|cookie|ip)/.test(normalizedKey);
  });
}

function auditDetailLabel(key: string) {
  const text = key.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/[._-]+/g, " ").trim();
  return text ? text.charAt(0).toLocaleUpperCase("pt-PT") + text.slice(1) : "Detalhe";
}
  ...
  return (
    <PageFrame title="Auditoria">
      ...
      <SimpleList
        items={listing.items}
        render={(item) => {
          const details = auditDetailEntries(item.details);
          return (
            <article className="operation" key={item.id}>
              <h3>{formatAuditActionLabel(item.action)}</h3>
              <dl>
                <div>
                  <dt>Quem</dt>
                  <dd>{item.actor.name || "Utilizador"}{item.actor.email ? ` · ${item.actor.email}` : ""}</dd>
                </div>
                <div><dt>Referência</dt><dd>{item.reference.label}</dd></div>
                <div><dt>Tipo</dt><dd>{formatEntityLabel(item.entity)}</dd></div>
                <div><dt>Data e hora</dt><dd>{formatPortugueseDateTime(item.createdAt)}</dd></div>
              </dl>
              {details.length > 0 ? (
                <details>
                  <summary>Ver detalhes mínimos</summary>
                  <dl>
                    {details.map(([key, value]) => (
                      <div key={key}>
                        <dt>{auditDetailLabel(key)}</dt>
                        <dd>{value === null ? "-" : typeof value === "boolean" ? (value ? "Sim" : "Não") : String(value)}</dd>
                      </div>
                    ))}
                  </dl>
                </details>
