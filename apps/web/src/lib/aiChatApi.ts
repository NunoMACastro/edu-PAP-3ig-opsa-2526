...
export interface AiChatMessage {
  ...
  payloadVersion?: 1 | 2;
  facts?: Array<{ id: string; metric: string; value: number | null; formattedValue: string; unit: string; sourceRef: string }>;
  provider: string;
  model?: string | null;
...
export interface AiCompletedMessage extends AiChatMessage {
  answer: string;
  claims: Array<{ metric: string; value: number | null; sourceRef: string }>;
  sourceRefs: string[];
}
