...
const HELP_BY_CODE: Record<string, string> = {
  API_ERROR: "Tenta novamente. Se o problema continuar, regista os detalhes técnicos e contacta a equipa técnica.",
  ...
  NETWORK_ERROR: "Confirma a ligação e se a API está disponível antes de repetir.",
  REQUEST_TIMEOUT: "Tenta novamente. Se continuar lento, reduz o intervalo ou contacta a equipa técnica.",
  SAFT_EXPORT_DISABLED: "A exportação SAF-T ainda não está disponível neste ambiente. Contacta um administrador.",
...
function formatUiMessage(message: UiErrorMessage) {
  return `${message.detail} Próxima ação: ${message.help}`;
}
