...
export interface StockBalanceItem {
  item: { id: string; sku: string; name: string; type: "PRODUCT" | "SERVICE" };
...
export function createApiClient(options: ApiClientOptions = {}) {
  ...
  return {
    ...
    inventory: {
      /** Lista o saldo atual por artigo/armazém com filtros company-scoped. */
      listStockBalances: (
        filters: CursorPagination & { itemId?: string; warehouseId?: string } = {},
      ) =>
        request<CursorPage<StockBalanceItem>>(
          "GET",
          `/inventory/stock-balances${queryString(filters)}`,
        ),
      /**
