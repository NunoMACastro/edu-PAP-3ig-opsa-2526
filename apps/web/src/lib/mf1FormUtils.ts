...
export function formatValue(value: unknown, columnName = ""): string {
  // A regra de apresentação fica centralizada para evitar formatos financeiros divergentes.
...
/**
 * Converte texto num inteiro maior ou igual a zero, adequado a taxas em basis points.
 *
 * @param value - Valor vindo do formulário.
 * @param label - Nome amigável usado na mensagem de erro.
 * @returns Inteiro não negativo validado.
 */
export function toNonNegativeInteger(
  ...
  if (!text || !Number.isInteger(parsed) || parsed < 0) {
    throw new Error(`${label} deve ser um número inteiro não negativo`);
  }
  return parsed;
}
