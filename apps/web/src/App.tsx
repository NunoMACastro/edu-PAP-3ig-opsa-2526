...
function DataTable({ rows }: { rows: ApiObject[] }) {
  ...
  return (
    <ResponsiveDataTable
      rows={safeRows}
      caption="Registos do modulo ativo"
      ...
      await action.run(
        async () => {
          ...
          if (validationErrors.length > 0) {
            // O BK-MF5-06 acrescenta proxima acao sem apagar as mensagens criadas no BK-MF5-05.
            throw new Error(formatMf5ValidationUiError(validationErrors));
          }

          let result: OperationResult;
          try {
            result = await operation.run(values);
          } catch (caught) {
            // Mesmo quando a API lanca ApiError, o texto passa por RNF06 antes do feedback visual.
            throw new Error(formatUiError(caught));
          }
