...
function OnboardingPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await action.run(
        async () => {
          await apiClient.onboarding.createCompany({
            ...
            profile: {
              ...
              fiscalYearStartDay: Number(form.get("fiscalYearStartDay")),
            },
          });
          const snapshot = await auth.refreshSession();
          if (!snapshot?.activeCompanyId) {
            throw new Error("A empresa foi criada, mas a sessão não recebeu o novo contexto.");
          }
          navigate("/companies", { replace: true });
        },
        {
          startMessage: "A criar empresa e perfil...",
          successMessage: "Empresa criada e ativada.",
          errorMessage: "Não foi possível concluir o onboarding.",
        },
      );
    } catch {
      // Os dados permanecem preenchidos para correção.
    }
  }
  ...
  return (
    <PageFrame
      ...
      description="Configura o contexto empresarial e fiscal necessário para usar os módulos."
    >
      <form className="operation" onSubmit={submit}>
        <div className="fields">
          <label><span>Nome da empresa</span><input name="name" required minLength={2} /></label>
          <label><span>Denominação legal</span><input name="legalName" required /></label>
          <label><span>NIF português</span><input name="nif" required inputMode="numeric" pattern="[0-9]{9}" /></label>
          <label><span>Morada</span><input name="addressLine1" required /></label>
          <label><span>Complemento de morada</span><input name="addressLine2" /></label>
          <label><span>Código postal</span><input name="postalCode" required autoComplete="postal-code" /></label>
          <label><span>Cidade</span><input name="city" required /></label>
          <label>
            <span>Mês de início fiscal</span>
            <input name="fiscalYearStartMonth" type="number" min={1} max={12} defaultValue={1} required />
          </label>
          <label>
            <span>Dia de início fiscal</span>
            <input name="fiscalYearStartDay" type="number" min={1} max={31} defaultValue={1} required />
          </label>
        </div>
        <button type="submit" disabled={action.busy}>
          {action.busy ? "A criar..." : "Criar empresa"}
        </button>
      </form>
      <ActionFeedbackMessage feedback={action.feedback} />
    </PageFrame>
...
function personOperations(
  ...
  return [
    ...
    },
    {
      ...
      submitLabel: "Atualizar",
      fields: [{ name: "id", label: "Registo", required: true }, ...personFields(requireNif)],
