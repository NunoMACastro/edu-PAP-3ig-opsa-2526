...
test("onboarding cria empresa, perfil, base operacional, ADMIN, sessão e auditoria numa transação", async () => {
    const calls = [];
    const tx = {
        ...
        fiscalPeriod: {
            create: async ({ data }) => {
                calls.push(["fiscalPeriod", data]);
                return { id: "period-1", ...data };
            },
        },
        account: {
            createMany: async ({ data }) => {
                calls.push(["accounts", data]);
                return { count: data.length };
            },
        },
        vatRate: {
            create: async ({ data }) => {
                calls.push(["vatRate", data]);
                return { id: "vat-1", ...data };
            },
        },
        warehouse: {
            create: async ({ data }) => {
                calls.push(["warehouse", data]);
                return { id: "warehouse-1", ...data };
            },
        },
