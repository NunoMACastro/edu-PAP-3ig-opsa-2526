...
test("BK-MF1-01: IVA isento exige e persiste código legal separado do motivo", () => {
    assert.throws(
        () => validateVatRateInput({
            code: "ISE",
            description: "Isento",
        ...
        { code: "MISSING_EXEMPTION_CODE" },
    );
    const result = validateVatRateInput({
        code: "ISE",
        description: "Isento",
        rateBps: 0,
        type: "EXEMPT",
    ...
    assert.equal(result.exemptionCode, "M07");
});
...
test("BK-MF1-02: venda calcula totais no backend e usa companyId do contexto", async () => {
    ...
    const prisma = {
        ...
        item: {
            findMany: async () => [
                { id: "item-1", name: "Servico", type: "SERVICE" },
            ],
        },
        vatRate: {
...
test("BK-MF1-02: emissão definitiva reserva número por upsert atómico", async () => {
    ...
    const prisma = {
        ...
        retentionHold: { upsert: async () => ({ id: "hold-sale-1" }) },
        auditLog: { create: async () => ({ id: "audit-1" }) },
        $transaction: async (callback) => callback(prisma),
    };
...
test("BK-MF1-03: recebimento definitivo protege registo e audit log", async () => {
    ...
    const prisma = {
        ...
        retentionHold: {
            upsert: async ({ create }) => {
                retainedEntities.push(create.entity);
                return { id: `hold-${retainedEntities.length}`, ...create };
            },
        },
        $transaction: async (callback) => callback(prisma),
    };
...
test("BK-MF1-04: lançamento de venda fica balanceado", async () => {
    ...
    const prisma = {
        ...
        saleDocument: {
            ...
            update: async ({ data }) => ({ id: "sale-1", ...data }),
        },
        account: {
        ...
        retentionHold: { upsert: async () => ({ id: "hold-entry-1" }) },
        auditLog: { create: async () => ({ id: "audit-1" }) },
        $transaction: async (callback) => callback(prisma),
    };
...
test("BK-MF1-05: títulos em aberto calculam antiguidade e ignoram liquidados", async () => {
    ...
    assert.equal(page.items[0].openAmountCents, 750);
...
test("BK-MF1-07/BK-MF1-10: compra nasce em rascunho com totais backend", async () => {
    ...
    const prisma = {
        ...
        item: {
            findMany: async () => [
                { id: "item-1", name: "Serviço", type: "SERVICE" },
            ],
        },
        vatRate: {
            findMany: async () => [{ id: "vat-1", rateBps: 2300 }],
...
test("BK-MF1-09: lançamento de compra fica balanceado", async () => {
    ...
    const prisma = {
        ...
        retentionHold: { upsert: async () => ({ id: "hold-purchase-1" }) },
        auditLog: {
            create: async ({ data }) => {
...
test("BK-MF1-10: compra paga pode ser lançada e termina em POSTED", async () => {
    ...
    const prisma = {
        ...
        retentionHold: { upsert: async () => ({ id: "hold-purchase-2" }) },
        auditLog: { create: async () => ({ id: "audit-1" }) },
        $transaction: async (callback) => callback(prisma),
    };
