...
/**
 * Acrescenta a unidade transacional e o delegate de auditoria exigidos pelos
 * fluxos AI persistentes, mantendo os doubles focados no comportamento MF4.
 *
 * @param {object} prisma - Double Prisma a completar.
 * @returns {object[]} Auditorias criadas pelo fluxo.
 */
function enableTransactionalAudit(prisma) {
    const audits = [];
    prisma.auditLog = {
        create: async ({ data }) => {
...
test("BK-MF4-01/BK-MF4-02: insights persistem fonte e sugestões não executam ações", async () => {
    ...
    enableTransactionalAudit(prisma);
...
test("BK-MF4-03: AiQuestionRun legado não aceita novas escritas", async () => {
    await assert.rejects(answerAiQuestion({}, { companyId: "company-1", userId: "user-1", body: { question: "Qual e a margem operacional recente?" } }), { code: "AI_QUESTION_RUN_LEGACY_READ_ONLY" });
});
