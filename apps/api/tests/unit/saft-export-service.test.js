...
function buildCreationFixture(period = SAFT_TEST_PERIOD, sources = buildSaftSources()) {
    const createdRuns = [];
    ...
    const prisma = {
        ...
        saleDocument: {
            findMany: async (query) => {
                observedQueries.saleDocuments = query;
                return sources.saleDocuments;
            },
        },
        purchaseDocument: {
            findMany: async (query) => {
                observedQueries.purchaseDocuments = query;
                return sources.purchaseDocuments;
            },
        },
        journalEntry: {
            findMany: async (query) => {
                observedQueries.journalEntries = query;
                return sources.journalEntries;
            },
        },
...
test("SAF-T aceita códigos raiz de um caráter definidos no plano contabilístico", () => {
    const sources = buildSaftSources();
    sources.accounts[0].code = "2";
    sources.accounts[1].code = "21";
    sources.accounts[1].saftGroupingCode = "2";
    sources.customers[0].saftAccountId = "21";
    sources.suppliers[0].saftAccountId = "21";

    const result = generateInternalSaft({
        sources,
        fiscalPeriod: SAFT_TEST_PERIOD,
        createdAt: new Date("2027-01-03T12:00:00.000Z"),
    });

    assert.match(result.artifact.toString("latin1"), /<AccountID>2<\/AccountID>/);
});

test("SAF-T académico limita um período aberto ao dia civil atual em Portugal", async () => {
    const openPeriod = {
        ...SAFT_TEST_PERIOD,
        status: "OPEN",
        closedAt: null,
    ...
    const {
        ...
        auditLogs,
        observedQueries,
    } = buildCreationFixture(openPeriod);
    let stored;
    const result = await createSaftExport(
        prisma,
        {
    ...
    assert.equal(result.openPeriodSnapshot, true);
    assert.equal(result.asOfDate, "2026-07-15");
    assert.equal(result.toDate, "2026-07-15");
    assert.equal(result.legalValidity, false);
    assert.match(result.warning, /sem validade legal/i);
    assert.equal(createdRuns[0].toDate.toISOString(), "2026-07-15T00:00:00.000Z");
    assert.deepEqual(
        {
            openPeriodSnapshot: createdRuns[0].validationDetails.openPeriodSnapshot,
            asOfDate: createdRuns[0].validationDetails.asOfDate,
            legalValidity: createdRuns[0].validationDetails.legalValidity,
            contractVersion: createdRuns[0].validationDetails.contractVersion,
        },
        {
            openPeriodSnapshot: true,
            asOfDate: "2026-07-15",
            legalValidity: false,
            contractVersion: "SAFT-ACADEMIC-SNAPSHOT-V1",
        },
    );
    assert.equal(
        observedQueries.saleDocuments.where.issuedAt.lt.toISOString(),
        "2026-07-16T00:00:00.000Z",
    );
    assert.equal(
        observedQueries.purchaseDocuments.where.issuedAt.lt.toISOString(),
        "2026-07-16T00:00:00.000Z",
    );
    assert.equal(
        observedQueries.journalEntries.where.entryDate.lt.toISOString(),
        "2026-07-16T00:00:00.000Z",
    );
    assert.match(stored.buffer.toString("latin1"), /<EndDate>2026-07-15<\/EndDate>/);
    assert.equal(stored.metadata["open-period-snapshot"], "true");
    assert.equal(stored.metadata["as-of-date"], "2026-07-15");
    assert.equal(stored.metadata["legal-validity"], "false");
    assert.equal(auditLogs[0].details.openPeriodSnapshot, true);
    assert.equal("artifact" in auditLogs[0].details, false);
    assert.equal("xml" in auditLogs[0].details, false);
});

test("snapshot académico aberto inclui uma compra contabilizada sem inventar rastreio de venda", async () => {
    const openPeriod = {
        ...SAFT_TEST_PERIOD,
        status: "OPEN",
        closedAt: null,
    };
    const purchaseSources = buildSaftSources();
    purchaseSources.saleDocuments = [];
    purchaseSources.journalEntries = purchaseSources.journalEntries.filter(
        (entry) => entry.source === "PURCHASE",
    );
    const { prisma, createdRuns } = buildCreationFixture(openPeriod, purchaseSources);
    let stored;
    const result = await createSaftExport(
        prisma,
        {
    ...
    const xml = stored.buffer.toString("latin1");
    assert.equal(result.status, "READY");
    assert.equal(createdRuns[0].saleDocumentCount, 0);
    assert.equal(createdRuns[0].purchaseDocumentCount, 1);
    assert.equal(createdRuns[0].journalEntryCount, 1);
    assert.match(xml, /Compra F 2026\/10/);
    assert.doesNotMatch(xml, /FT 2026\/1/);
    assert.match(xml, /DEMONSTRACAO ACADEMICA - NAO CERTIFICADO/);
});

test("SAF-T externo continua a rejeitar um período aberto", async () => {
    const openPeriod = {
        ...SAFT_TEST_PERIOD,
        status: "OPEN",
        closedAt: null,
    };
    const { prisma, createdRuns } = buildCreationFixture(openPeriod);
    let storageCalls = 0;
    await assert.rejects(
        () => createSaftExport(
            prisma,
            ...
            creationInput(true, "external"),
            {
                ...
                externalPipeline: { validate: async () => assert.fail("não deve validar") },
                now: new Date("2026-07-14T23:30:00.000Z"),
            },
        ),
        { code: "SAFT_FISCAL_PERIOD_NOT_CLOSED", status: 409 },
    );
