 ...
 * @returns {Promise<string>} Conteúdo XLSX codificado em base64.
 */
async function workbookBase64(headers, rows) {
    ...
    const tx = {
        customer: {
            upsert: async ({ where, create, update }) => {
                assert.equal(where.companyId_nif.companyId, "company-real");
                assert.equal(create.companyId, "company-real");
                assert.equal(update.name, "Cliente XLSX");
                created.customers.push({ where, create, update });
                return { id: "customer-1", ...create };
            },
        },
        businessImportRun: {
            create: async ({ data }) => {
