    ...
    const transactionClient = {
        alertPreference: {
            ...
            async findUnique(args) {
                ...
                return storedPreferences.find(
                    (preference) =>
                        preference.type ===
                        args.where.companyId_userId_type.type,
                ) ?? null;
            },
            async upsert(args) {
        ...
        auditLog: {
            async create(args) {
                ...
                return { id: "audit-mf8-12", ...args.data };
            },
        },
    };
...
test("tipos de alerta IA não são expostos nem alteráveis por roles sem AI_ALERTS_READ", async () => {
    const { prisma } = createPreferencePrismaDouble();
    const preferences = await listAlertPreferences(prisma, { companyId: COMPANY_ID, userId: USER_ID, role: "OPERACIONAL" });
    assert.equal(preferences.some((preference) => preference.type === "ai"), false);
    await assert.rejects(
        setAlertPreference(prisma, { companyId: COMPANY_ID, userId: USER_ID, role: "OPERACIONAL", type: "ai", enabled: false }),
        hasDomainCode("FORBIDDEN"),
    );
});
