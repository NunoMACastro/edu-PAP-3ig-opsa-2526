...
test("BK-MF8-02: router expoe GET /", () => {
...
test("BK-MF8-02: GET / devolve payload publico seguro", async () => {
    const router = buildHealthRoutes({ version: "1.0.0", environment: "test" });
...
test("BK-MF8-02: payload usa relogio controlado nos testes", () => {
...
test("BK-MF8-02: falha sem versao configurada", () => {
    assert.throws(
        ...
        /version e obrigatorio/,
    );
});

test("BK-MF8-02: falha com ambiente desconhecido", () => {
    assert.throws(
        () => buildHealthPayload({ version: "1.0.0", environment: "staging" }),
        /Ambiente de health-check invalido/,
    );
});

test("BK-MF8-02: router rejeita configuracao invalida antes de expor readiness", () => {
    assert.throws(
        () => buildHealthRoutes({ version: "1.0.0", environment: "staging" }),
        /Ambiente de health-check invalido/,
    );
});
