    ...
    const prisma = {
        companySubscription: {
            ...
            async updateMany(args) {
                ...
                if (
                    !storedSubscription ||
                    storedSubscription.id !== args.where.id ||
                    storedSubscription.companyId !== args.where.companyId ||
                    storedSubscription.status !== args.where.status ||
                    storedSubscription.updatedAt.getTime() !== args.where.updatedAt.getTime()
                ) {
                    return { count: 0 };
                }
                storedSubscription = {
...
test("lifecycle devolve STALE_STATE sem auditoria quando perde o claim concorrente", async () => {
    const { prisma, calls } = createLifecyclePrisma(createSubscription());
    prisma.companySubscription.updateMany = async (args) => {
        calls.update.push(args);
        return { count: 0 };
    };
