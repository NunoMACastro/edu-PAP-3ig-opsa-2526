        ...
        try {
            ...
            const updatedManualJournal = await updateManualJournal(
                ...
                manualJournal.id,
                {
                    ...
                    reason: "Correção validada pelo teste de integração",
                    lines: [
            ...
            const sourcePath = path.join(storageRoot, "attachment-source.pdf");
            await writeFile(sourcePath, attachmentContent);
            const objectStorage = new LocalObjectStorage(
                path.join(storageRoot, "objects"),
            );
            const attachment = await addManualJournalAttachment(
                ...
                manualJournal.id,
                {
                    ...
                    sha256: createHash("sha256")
                        ...
                        .digest("hex"),
                    head: attachmentContent,
                    tempPath: sourcePath,
                },
            );
            const attachmentRecord = await prisma.journalAttachment.findUnique({
            ...
            const storedAttachment = await readFile(
                path.join(
                    storageRoot,
                    "objects",
                    ...attachmentRecord.storageKey.split("/"),
                ),
            );
