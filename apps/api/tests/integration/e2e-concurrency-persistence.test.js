...
test(
    ...
    async (t) => {
        ...
        try {
            ...
            await t.test("duas publicações de uma contagem multi-linha criam um ajuste por linha", async () => {
                ...
                const [item, secondItem] = await Promise.all([
                    prisma.item.create({
                        data: {
                            companyId: company.id,
                            sku: `${prefix}-ITEM-A`,
                            name: "Primeiro artigo da contagem concorrente",
                            type: "PRODUCT",
                            costCents: 125,
                            priceCents: 200,
                            vatRateBps: 2300,
                    ...
                    prisma.item.create({
                        data: {
                            companyId: company.id,
                            sku: `${prefix}-ITEM-B`,
                            name: "Segundo artigo da contagem concorrente",
                            type: "PRODUCT",
                            costCents: 150,
                            priceCents: 240,
                            vatRateBps: 2300,
                ...
                try {
                    ...
                    await createStockMovement(prisma, company.id, user.id, {
                        ...
                        unitCostCents: 150,
                        toWarehouseId: warehouse.id,
                        reason: "Segunda entrada para a contagem concorrente",
                    });
                    const count = await createInventoryCount(
                        ...
                        user.id,
                        {
                            ...
                            lines: [
                                { itemId: item.id, countedQuantity: 4 },
                                { itemId: secondItem.id, countedQuantity: 6 },
                            ],
                        },
                    );
                    await createStockMovement(prisma, company.id, user.id, {
                    ...
                    await createStockMovement(prisma, company.id, user.id, {
                        ...
                        unitCostCents: 150,
                        toWarehouseId: warehouse.id,
                        reason: "Segunda entrada posterior ao snapshot da contagem",
                    });
                    ...
                    const [persisted, movements, balances, audits] = await Promise.all([
                        prisma.inventoryCount.findUnique({ where: { id: count.id } }),
                        prisma.stockMovement.findMany({
                            where: {
                                companyId: company.id,
                                sourceType: "INVENTORY_COUNT_LINE",
                                sourceId: { in: count.lines.map((line) => line.id) },
                            },
                        }),
                        prisma.stockBalance.findMany({
                            where: {
                                companyId: company.id,
                                itemId: { in: [item.id, secondItem.id] },
                                warehouseId: warehouse.id,
                            },
                        }),
                    ...
                    assert.equal(new Set(movements.map(({ sourceId }) => sourceId)).size, 2);
                    assert.deepEqual(
                        new Map(balances.map((balance) => [
                            balance.itemId,
                    ...
                    assert.equal(audits[0].details.adjustments.length, 2);
                } finally {
                    await cleanupScenario(prisma, company.id, [user.id]);
                }
            });
