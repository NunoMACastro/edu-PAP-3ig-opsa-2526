...
import { buildSaftExport } from "../../src/modules/compliance/saftService.js";
...
test(
    ...
    async () => {
        ...
        try {
            ...
            const [vatMap, forecast, businessImport, saft, operational, kpis] =
                await Promise.all([
                    ...
                    importBusinessData(prisma, {
                        ...
                        input: {
                            ...
                            content: "name;nif;email\nCliente Importado;;importado@example.test",
                        },
                    }),
                    buildSaftExport(prisma, { companyId: company.id, userId: user.id, fromDate, toDate }),
            ...
            assert.match(saft.xml, /<AuditFile>/);
            ...
            assert.equal(await prisma.saftExportRun.count({ where: { companyId: company.id } }), 1);
