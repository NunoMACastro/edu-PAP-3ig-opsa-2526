 ...
 * BK-MF8-18 report. It validates a clean corrected path, a
 * no-correction-needed path, or an explicit environment blocker when the
 * affected command cannot be revalidated without a safe ephemeral database.
 * The CLI still fails closed for the blocked path so it can be used as a
 * release gate without turning an honest blocker into a false green.
 */
...
/**
 * Rejects a validated report that still represents an active release blocker.
 *
 * Validation and gate readiness are intentionally separate: a blocked report
 * can be structurally valid evidence, but it cannot make the academic release
 * gate pass.
 *
 * @param {{ decision: string }} result Validated defect-report summary.
 * @returns {void}
 * @throws {Error} When the report keeps an environment blocker active.
 */
export function assertDefectReportGateCanPass(result) {
    if (result.decision === ENV_BLOCKED_DECISION) {
        throw new Error(
            "MF8_DEFECT_REPORT_BLOCKED: a decisão BLOQUEADO_AMBIENTE exige exit diferente de zero."
        );
    }
}
...
if (import.meta.url === executedFileUrl) {
    try {
        ...
        assertDefectReportGateCanPass(result);
        console.log(`BK-MF8-18 validado: ${result.decision} (${result.rerunCommand})`);
    } catch (error) {
        const message = error instanceof Error ? error.message : "erro desconhecido";
        console.error(`[mf8:defect-report] FAIL: ${message}`);
        process.exitCode = 1;
    }
}
