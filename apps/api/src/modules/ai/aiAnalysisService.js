...
function buildCandidates(metrics, rules, period) {
    const candidates = [];
    const add = (ruleCode, condition, value, score, content, evidence) => {
        const rule = rules[ruleCode];
        if (!rule.enabled || !condition) return;
        const materialBucket = Math.round(Number(value ?? 0) / Math.max(1, Math.abs(rule.threshold) || 10_000));
        const fp = fingerprint(ruleCode, materialBucket);
        candidates.push({
            ruleCode,
            ruleVersion: rule.version,
            fingerprint: fp,
            sourceType: "AI_METRIC_V2",
            sourceId: `${ruleCode}:${fp.slice(0, 16)}`,
...
function actionTypeForCandidate(candidate) {
    return candidate.type === "LOW_MARGIN" ? "REVIEW_PRICING"
        : candidate.type === "CUSTOMER_COLLECTION_RISK" ? "NEGOTIATE_CUSTOMER"
            : candidate.type === "STOCK_RISK" ? "REVIEW_STOCK"
                : "REVIEW_CASHFLOW";
}
...
export async function processAnalysisRun(prisma, run, options = {}) {
    ...
    try {
        ...
        const candidates = buildCandidates({ cashflow, receivables, stock, margin }, rules, run);
