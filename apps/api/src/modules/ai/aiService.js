/**
 * @file Services da IA assistiva MF4.
 *
...
const EXPLAINABLE_INSIGHT_FIELDS = Object.freeze([
    "title",
    "explanation",
    "sourceType",
    "sourceId",
...
/**
 * Valida o contrato minimo de explicabilidade exigido por RNF31.
 *
 ...
 * @returns {void}
 * @throws {import("../../lib/httpErrors.js").HttpError} Quando faltam explicacao ou fonte rastreavel.
 */
export function assertExplainableInsight(insight) {
    if (!insight || typeof insight !== "object") {
        throw httpError(
            422,
            "AI_INSIGHT_NOT_EXPLAINABLE",
            "Insight sem explicabilidade minima",
            { missing: EXPLAINABLE_INSIGHT_FIELDS },
        );
    }

    const missing = EXPLAINABLE_INSIGHT_FIELDS.filter((field) => {
        return typeof insight[field] !== "string" || insight[field].trim().length === 0;
    });
    ...
    if (insight.explanation.trim().length < 40) {
        throw httpError(
            422,
            "AI_INSIGHT_EXPLANATION_TOO_SHORT",
            "Explicacao do insight demasiado curta",
            { minimumLength: 40 },
        );
    }
}
...
export async function generateAiInsights(prisma, input) {
    ...
    for (const candidate of candidates) {
        // O RNF31 deve falhar antes da persistencia: um insight sem fonte nao e defensavel.
        assertExplainableInsight(candidate);
        insights.push(
...
export async function generateAiSuggestions(prisma, input) {
    ...
    for (const insight of insights) {
        ...
        suggestions.push({
            ...suggestion,
            sourceQuality,
            guardrail: "A IA recomenda com fonte rastreavel; a decisao continua humana.",
        });
    }
    return suggestions;
}
...
/**
 * Responde a perguntas de leitura com dados e fontes internas.
 *
 * @param {import("@prisma/client").PrismaClient} prisma - Cliente Prisma.
...
export async function answerAiQuestion(prisma, input) {
    const payload = validateQuestionPayload(input.body);
    const latestReport = await prisma.operationalReportRun.findFirst({
        where: { companyId: input.companyId },
        orderBy: { generatedAt: "desc" },
    });
    const latestKpis = await prisma.executiveKpiRun.findFirst({
        where: { companyId: input.companyId },
        orderBy: { generatedAt: "desc" },
    });
    const latestForecast = await prisma.cashflowForecastRun.findFirst({
        where: { companyId: input.companyId },
        orderBy: { generatedAt: "desc" },
    });

    const sources = [];
    let answer = "Ainda nao existem dados suficientes para responder com seguranca.";
    if (payload.intent === "cashflow" && latestForecast) {
        answer = `A previsao mais recente fecha em ${eur(latestForecast.closingBalanceCents)}.`;
        sources.push({ type: "CashflowForecastRun", id: latestForecast.id });
    } else if (payload.intent === "margem" && latestReport) {
        answer = `A margem operacional MVP mais recente e ${eur(latestReport.marginCents)}.`;
        sources.push({ type: "OperationalReportRun", id: latestReport.id });
    } else if (payload.intent === "stock" && latestReport) {
        answer = `O valor de stock do ultimo relatorio operacional e ${eur(latestReport.stockValueCents)}.`;
        sources.push({ type: "OperationalReportRun", id: latestReport.id });
    } else if (payload.intent === "kpis" && latestKpis) {
        answer = `EBITDA MVP ${eur(latestKpis.ebitdaCents)}; PMR ${latestKpis.pmrDays ?? "sem dados"}; PMP ${latestKpis.pmpDays ?? "sem dados"}.`;
        sources.push({ type: "ExecutiveKpiRun", id: latestKpis.id });
    } else if (payload.intent === "clientes") {
        const openDocuments = await prisma.saleDocument.findMany({
            where: {
                companyId: input.companyId,
                kind: { not: "CREDIT_NOTE" },
                status: { in: ["ISSUED", "SETTLED"] },
        ...
        const totalOpenCents = openDocuments.reduce(
            (sum, document) => sum + Math.max(0, document.totalCents - document.amountPaidCents),
            0,
        );
        answer = `Existem ${eur(totalOpenCents)} em documentos de cliente por receber.`;
        sources.push({ type: "SaleDocument", id: "open-items" });
    }

    const run = await prisma.aiQuestionRun.create({
        data: {
            companyId: input.companyId,
