...
async function getStockRiskSummary(prisma, companyId, period) {
    ...
    const [summaryRows, risks] = isCurrent ? await Promise.all([
        prisma.$queryRaw`
            WITH risks AS (
    ...
    return resultEnvelope("get_stock_risk_summary", period, {
        ...
        formula: isCurrent ? "StockBalance versus configured thresholds" : "sum movements into/out of warehouse up to period end",
        counts: { settingCount: Number(summary.settingCount ?? 0), movementCount },
        ...
        limitations: !isCurrent && movementCount === 0
            ...
            : !isCurrent
                ...
                : [],
        sourceRefs: risks.map((risk) => ({ type: "ITEM", entityType: "ITEM", entityId: risk.itemId, warehouseId: risk.warehouseId, riskType: risk.type })),
    });
}
