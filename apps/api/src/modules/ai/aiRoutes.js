...
export function buildAiRoutes({ prisma, apiEnv, redisClient = null, providerOverride = null }) {
    ...
    function parseAuthorizedTransaction(req, rawTransaction) {
        const parsed = parseTransactionAnalysisInput(rawTransaction);
        const permission = parsed.documentType === "SALE"
            ? Permission.SALES_READ
            : Permission.PURCHASES_READ;
        if (!hasPermission(req.role, permission)) {
            throw httpError(403, "PERMISSION_FORBIDDEN", "Permissão insuficiente");
        }
        return parsed;
    }

    /** Exige a permissão de leitura do domínio indicado no body já validado. */
    function transactionReadPermissionMiddleware(req, res, next) {
        try {
            const parsed = parseAuthorizedTransaction(req, req.body);
            req.aiTransactionInput = parsed;
            return next();
        } catch (error) { return sendError(res, error); }
    }

    /** Aplica o mesmo RBAC quando o chat recebe uma transação selecionada. */
    function chatTransactionReadPermissionMiddleware(req, res, next) {
        const rawTransaction = req.body?.context?.transaction;
        if (rawTransaction == null) return next();
        try {
            const parsed = parseAuthorizedTransaction(req, rawTransaction);
            req.body = {
                ...req.body,
                context: { ...req.body.context, transaction: parsed },
            };
            return next();
        } catch (error) { return sendError(res, error); }
    }
    ...
    router.post("/transaction-analysis", analysisGuards, transactionReadPermissionMiddleware, async (req, res) => {
        try {
            await reserveManualAnalysis({ companyId: req.companyId, userId: req.user.id });
            const result = await analyzeTransaction(prisma, {
                companyId: req.companyId,
                ...req.aiTransactionInput,
            });
    ...
    router.post("/chat/sessions/:id/messages", chatGuards, chatTransactionReadPermissionMiddleware, async (req, res) => {
    ...
    router.post("/questions", chatGuards, chatTransactionReadPermissionMiddleware, async (req, res) => {
