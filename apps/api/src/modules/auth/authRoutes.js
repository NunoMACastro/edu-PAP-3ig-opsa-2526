 ...
 *
 * @param {{ prisma: import("@prisma/client").PrismaClient, isProduction: boolean, appBaseUrl: string }} deps - Dependências backend do router.
...
export function buildAuthRoutes({
    ...
    const router = Router();
    const controller = buildAuthController({ prisma, isProduction, rateLimiter });
    const passwordResetController = buildPasswordResetController({
        prisma,
        emailAdapter: buildPasswordResetEmailAdapter({ appBaseUrl, emailOutbox }),
        rateLimiter,
    });
