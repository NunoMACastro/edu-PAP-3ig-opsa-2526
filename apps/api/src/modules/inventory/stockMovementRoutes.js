...
export function buildStockMovementRoutes({ prisma }) {
    ...
    router.get("/stock-movements", readGuards, async (req, res) => {
        try {
            const result = await listStockMovements(prisma, req.companyId, {
    ...
    router.get("/stock-balances", readGuards, async (req, res) => {
        try {
            const result = await listStockBalances(prisma, req.companyId, {
