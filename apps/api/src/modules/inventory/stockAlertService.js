...
export async function saveStockAlertSetting(prisma, context) {
    ...
    const [item, warehouse] = await Promise.all([
        prisma.item.findFirst({
            where: { id: data.itemId, companyId: context.companyId, isActive: true },
        }),
        prisma.warehouse.findFirst({
            where: { id: data.warehouseId, companyId: context.companyId, isActive: true },
        }),
    ]);
    ...
    const setting = await prisma.stockAlertSetting.upsert({
        where: {
