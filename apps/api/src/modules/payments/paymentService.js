...
export async function registerPayment(
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (document.kind === "SUPPLIER_CREDIT_NOTE") {
            throw httpError(409, "CREDIT_NOTE_NOT_PAYABLE", "Notas de crédito não recebem pagamentos");
        }

        if (!["APPROVED", "POSTED", "PAID"].includes(document.status)) {
            throw httpError(409, "INVALID_STATUS", "Apenas compras aprovadas ou lançadas podem receber pagamentos");
        }
        ...
        await tx.purchaseDocument.update({
            ...
            data: {
                ...
                status: nextPaid === document.totalCents ? "PAID" : document.status,
            },
        });
