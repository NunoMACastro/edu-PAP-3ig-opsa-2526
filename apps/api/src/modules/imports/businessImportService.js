/**
 * @file Service de importação CSV de clientes, fornecedores, artigos e extratos.
 */
...
} from "./businessImportValidators.js";
 ...
 *
 * Excel nativo fica fora desta MF; o contrato aceito é CSV exportado do Excel.
 *
...
export async function importBusinessData(prisma, context) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (data.type === "STATEMENTS") {
            ...
            const rows = parseCsvRows(data.content);
        ...
        await tx.auditLog.create({
            data: {
                ...
                entityId: run.id,
                details: { type: data.type, acceptedRows, rejectedRows },
            },
        });
        await recordIntegrationLog(tx, {
            ...
            message: "Importacao de dados de negocio concluida com validacao por linha.",
        });
