...
export async function buildVatMap(prisma, input) {
    return prisma.$transaction(async (tx) => {
        ...
        const run = await tx.vatMapRun.create({
            data: {
                companyId: input.companyId,
        ...
        await upsertRetentionHold(tx, {
            ...
            entity: "VatMapRun",
            entityId: run.id,
            periodEndAt: input.toDate,
            reason: "VAT_MAP_GENERATED",
        });
        await recordRetainedAuditLog(tx, {
            ...
            entityId: run.id,
            periodEndAt: input.toDate,
            retentionReason: "VAT_MAP_AUDIT_RETAINED",
            details: {
                fromDate: input.fromDate.toISOString(),
                toDate: input.toDate.toISOString(),
                sourceEntries: journalEntries.length,
                rows: rows.length,
            },
        });
