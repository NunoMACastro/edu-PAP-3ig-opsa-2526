...
import { recordRetainedSensitiveAudit } from "../audit/auditLogService.js";
    ...
    if (!warehouseId) {
        if (required) {
            throw httpError(
                400,
                "WAREHOUSE_REQUIRED",
                "Indica o armazém para uma venda com produtos",
            );
        }
        return null;
    }
...
/**
 * Obtém o detalhe de uma venda pertencente à empresa ativa.
 *
 * @param {import("@prisma/client").PrismaClient} prisma - Cliente Prisma.
 * @param {string} companyId - Empresa ativa obtida da sessão.
 ...
 */
export async function getSaleDocument(prisma, companyId, id) {
...
export async function issueSaleDocument(prisma, companyId, userId, id) {
    return prisma.$transaction(async (tx) => {
        ...
        await recordRetainedSensitiveAudit(tx, {
            ...
            retentionReason: "SALE_DOCUMENT_ISSUE_AUDIT_RETAINED",
            details: {
                result: "success",
                number,
