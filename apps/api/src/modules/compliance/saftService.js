/**
 * @file Gerador SAF-T (PT) MVP da MF3 reforcado com readiness check MF7.
 */
 ...
 *
 * @param {object[]} journalEntries - Lancamentos contabilisticos a exportar.
 * @returns {string} Bloco XML com lancamentos contabilisticos SAF-T.
 */
...
/**
 * Constroi o input de readiness a partir dos dados ja lidos pelo service.
 *
 ...
 */
function buildSaftReadinessInput({
    input,
    profile,
    saleDocuments,
    purchaseDocuments,
...
export async function buildSaftExport(prisma, input) {
    ...
    // So depois da readiness check criamos nome, XML, run persistente e log de sucesso.
...
<AuditFile>
  ...
  <SourceDocuments>${sourceDocumentsXml(saleDocuments, purchaseDocuments)}
    ...
    return {
        ...
        note: "SAF-T MVP rastreavel; nao substitui validacao legal oficial.",
    };
}
