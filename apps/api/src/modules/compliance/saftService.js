...
const ACADEMIC_SNAPSHOT_CONTRACT_VERSION = "SAFT-ACADEMIC-SNAPSHOT-V1";
const ACADEMIC_WARNING = "Demonstração académica sem validade legal e não certificada.";
...
/**
 * Resolve o relógio de referência injetável usado pelo snapshot académico.
 *
 * @param {Date | string | undefined} value - Relógio opcional da operação.
 * @returns {Date} Data válida e defensivamente copiada.
 */
function referenceDate(value) {
    const date = value == null ? new Date() : new Date(value);
    if (Number.isNaN(date.getTime())) {
        throw httpError(500, "INVALID_SAFT_CLOCK", "Relógio SAF-T inválido.");
    }
    return date;
}

/**
 * Obtém o dia civil corrente em Portugal sem depender do timezone do processo.
 * A data UTC resultante representa apenas o dia civil, não um instante local.
 *
 * @param {Date} now - Instante de referência.
 * @returns {Date} Dia civil Europe/Lisbon às 00:00:00Z.
 */
function currentLisbonCivilDate(now) {
...
/**
 * Carrega um período pertencente à empresa ativa. O modo externo mantém a
 * exigência de fecho. Só o modo académico fora de produção pode criar um
 * snapshot de um período aberto, limitado ao dia civil atual em Portugal.
 *
 * @param {import("@prisma/client").PrismaClient} prisma - Cliente Prisma.
 * @param {{ companyId: string, fiscalPeriodId: string }} input - Ownership backend.
 * @param {"ACADEMIC" | "EXTERNAL"} validationMode - Modo já normalizado.
 * @param {Date} now - Relógio validado da operação.
 * @returns {Promise<{ period: object, effectivePeriod: object, openPeriodSnapshot: boolean, asOfDate: string | null }>} Período original e intervalo efetivo.
 */
async function loadFiscalPeriodForExport(prisma, input, validationMode, now) {
    ...
    const isClosed = period.status === "CLOSED" && Boolean(period.closedAt);
    if (isClosed) {
        return {
            period,
            effectivePeriod: period,
            openPeriodSnapshot: false,
            asOfDate: null,
        };
    }
    if (validationMode !== ACADEMIC_MODE || period.status !== "OPEN") {
        throw httpError(
            409,
    ...
    const currentLocalDate = currentLisbonCivilDate(now);
    const periodStart = new Date(period.startDate);
    const periodEnd = new Date(period.endDate);
    const effectiveEndDate = periodEnd.getTime() < currentLocalDate.getTime()
        ? periodEnd
        : currentLocalDate;
    if (effectiveEndDate.getTime() < periodStart.getTime()) {
        throw httpError(
            409,
            "SAFT_ACADEMIC_SNAPSHOT_BEFORE_PERIOD",
            "O período fiscal ainda não começou na data de corte académica.",
        );
    }
    return {
        period,
        effectivePeriod: { ...period, endDate: effectiveEndDate },
        openPeriodSnapshot: true,
        asOfDate: dateOnly(effectiveEndDate),
    };
}
...
function serializeSaftRun(run, activeMode = EXTERNAL_MODE) {
    ...
    return {
        ...
        validationMode,
        certified,
        legalValidity: validationMode === ACADEMIC_MODE ? false : null,
        openPeriodSnapshot: run?.validationDetails?.openPeriodSnapshot === true,
        asOfDate: run?.validationDetails?.asOfDate ?? null,
        fromDate: run?.fromDate ? dateOnly(run.fromDate) : null,
        toDate: run?.toDate ? dateOnly(run.toDate) : null,
        warning: validationMode === ACADEMIC_MODE ? ACADEMIC_WARNING : null,
...
export async function createSaftExport(
    ...
    const now = referenceDate(dependencies.now);
    const {
        period,
        effectivePeriod,
        openPeriodSnapshot,
        asOfDate,
    } = await loadFiscalPeriodForExport(prisma, input, validationMode, now);
    ...
    const fileName = validationMode === ACADEMIC_MODE
        ? `saft-DEMO-NAO-CERTIFICADO-${safeNif}-${dateOnly(effectivePeriod.startDate)}-${dateOnly(effectivePeriod.endDate)}.xml`
        : `saft-${safeNif}-${dateOnly(effectivePeriod.startDate)}-${dateOnly(effectivePeriod.endDate)}.xml`;
    ...
    const stored = await objectStorage.putBuffer({
        ...
        metadata: {
            ...
            ...(validationMode === ACADEMIC_MODE
                ? {
                    "open-period-snapshot": String(openPeriodSnapshot),
                    "as-of-date": asOfDate ?? dateOnly(effectivePeriod.endDate),
                    "legal-validity": "false",
                    "snapshot-contract-version": ACADEMIC_SNAPSHOT_CONTRACT_VERSION,
                }
                : {}),
    ...
    try {
        run = await prisma.$transaction(async (tx) => {
                ...
                data: {
                    ...
                    validationDetails: {
                        ...
                        ...(validationMode === ACADEMIC_MODE
                            ? {
                                openPeriodSnapshot,
                                asOfDate: asOfDate ?? dateOnly(effectivePeriod.endDate),
                                legalValidity: false,
                                contractVersion: ACADEMIC_SNAPSHOT_CONTRACT_VERSION,
                            }
                            : {}),
                        schema: {
                            ...(schemaIdentity?.sha256
            ...
            await upsertRetentionHold(tx, {
                ...
                entityId: created.id,
                periodEndAt: effectivePeriod.endDate,
