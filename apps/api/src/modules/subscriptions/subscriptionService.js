...
export async function runSimulatedSubscriptionAction(prisma, input) {
    ...
    const subscription = await prisma.$transaction(async (tx) => {
        ...
        // A leitura acontece antes do lock para que dois pedidos que observaram
        // a mesma versão concorram pelo mesmo `updatedAt`. Depois do lock, só o
        // primeiro claim pode vencer; pedidos realmente posteriores leem a nova
        // versão e continuam a ser tratados como operações sequenciais normais.
        await acquireTransactionLock(
            tx,
            "company-subscription",
            lifecycleInput.companyId,
        ...
        const claim = await tx.companySubscription.updateMany({
            where: {
        ...
        if (claim.count !== 1) {
            throw httpError(
                409,
                "STALE_STATE",
                "A subscrição foi alterada por outra operação. Atualiza os dados e tenta novamente.",
            );
        }
        const savedSubscription = await tx.companySubscription.findUnique({
            where: { companyId: lifecycleInput.companyId },
        });
        if (!savedSubscription) {
            throw httpError(
                409,
                "STALE_STATE",
                "A subscrição deixou de estar disponível durante a alteração.",
            );
        }
