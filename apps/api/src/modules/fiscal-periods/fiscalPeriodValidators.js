 ...
 * @returns {{ name: string, startDate: Date, endDate: Date, fiscalYear?: number }} Payload normalizado.
 */
export function validateFiscalPeriodPayload(body) {
    ...
    let fiscalYear;
    if (Object.hasOwn(body, "fiscalYear")) {
        if (
            !Number.isInteger(body.fiscalYear) ||
            body.fiscalYear < 1900 ||
            body.fiscalYear > 9999
        ) {
            throw httpError(
                400,
                "INVALID_FISCAL_YEAR",
                "fiscalYear deve ser um exercício fiscal explícito entre 1900 e 9999",
            );
        }
