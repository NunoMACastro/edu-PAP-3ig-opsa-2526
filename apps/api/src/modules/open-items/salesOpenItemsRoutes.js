...
/**
 * Constrói o router de títulos de venda em aberto.
 * Liga a consulta autorizada ao service que calcula saldos por data de referência.
 *
...
export function buildSalesOpenItemsRoutes({ prisma }) {
    ...
    router.get("/", guards, async (req, res) => {
        try {
            const page = await listSalesOpenItems(
                prisma,
                req.companyId,
                req.query,
            );
            return res.status(200).json({
                ...
                // Alias transitório para consumidores anteriores ao envelope comum.
