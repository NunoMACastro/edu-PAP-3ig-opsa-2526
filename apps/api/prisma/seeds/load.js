...
export async function seedLoadProfile(prisma, input) {
    ...
    await prisma.account.createMany({
        data: [
            ["11", "Caixa"], ["12", "Depositos"], ["211", "Clientes"],
            ["221", "Fornecedores"], ["2432", "IVA dedutivel"],
            ["2433", "IVA liquidado"], ["62", "Gastos"], ["72", "Rendimentos"],
        ].map(([code, name]) => ({ companyId: company.id, code, name, level: code.length })),
    });
    const accounts = Object.fromEntries(
        (await prisma.account.findMany({ where: { companyId: company.id } })).map((row) => [row.code, row]),
    );
    const vat = await prisma.vatRate.create({
        data: { companyId: company.id, code: "LOAD23", description: "IVA 23%", rateBps: 2300, type: "NORMAL" },
    });
