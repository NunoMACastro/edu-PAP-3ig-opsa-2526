...
async function verifyDocumentInvariants(prisma, companyId) {
    const [sales, purchases] = await Promise.all([
        prisma.saleDocument.findMany({ where: { companyId }, include: { receipts: true } }),
        prisma.purchaseDocument.findMany({ where: { companyId }, include: { payments: true } }),
    ]);
...
async function verifyFiscalAndAi(prisma, companyId) {
    const [closedPeriods, openPeriods, insights] = await Promise.all([
